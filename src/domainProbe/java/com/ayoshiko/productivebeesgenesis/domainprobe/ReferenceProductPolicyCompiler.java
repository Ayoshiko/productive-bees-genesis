...
/** 77db5b5 的编译算法，仅作为开发源集的独立回归对照，不进入发行 JAR。 */
public final class ReferenceProductPolicyCompiler {
	public record Result(ProductPolicySnapshot snapshot, List<String> diagnostics, List<ProductKey> keys) {
		public Result { diagnostics = List.copyOf(diagnostics); }
	}
	private ReferenceProductPolicyCompiler() { }
	...
	public static Result compile(ServerLevel level, long revision) {
		...
		return new Result(new ProductPolicySnapshot(revision, descriptors, List.of()), diagnostics, descriptors.stream().map(AllowedProductDescriptor::template).distinct().toList());
	}
