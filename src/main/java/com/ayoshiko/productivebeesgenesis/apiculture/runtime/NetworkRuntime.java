...
public final class NetworkRuntime {
	public enum Status { STOPPED, PREPARING, RUNNING, ENERGY, FLOWER, ENVIRONMENT, STALE_PLAN, UNAVAILABLE, UNSUPPORTED, SETTLING, BACKLOG, FAILED }
	...
	long step(NetworkCoreBlockEntity core, long now) {
		...
		if (record == null || record.phase() != OwnedMachineRecord.Phase.OWNED) return nextTick(now);
