...
/**
	 * MEK 离心机统一标记接口。
	 * 供本模组专用弹出组件统一处理所有离心机类型，
	 * 避免硬依赖 ME/EME 可选模组类引发 ClassNotFoundException。
	 * <p>
	 * Task 16: 暴露输出槽内容版本号，供专用 Ejector 在输出槽内容未变化时跳过
	 * 高频的 outputItems 调用，降低 TimeWand 高倍加速下的 CPU 开销。
	 * <p>
	 * Step 5: 暴露输出槽物品总数（O(1) 读取），供专用 Ejector 替代
...
public interface IMekCentrifugeTile {
	 ...
	 * 输出槽内容版本号。
	 * <br/>
	 * 每次输出槽内容发生变化时应递增；专用 Ejector 通过比较版本号解除阻塞退避。
	 * 服务端单线程访问，实现类可用 volatile 保证可见性。
	 */
	...
	/**
	 * 返回所有输出槽的物品总数，供专用 Ejector 替代 O(processes×3) 遍历计数。
	 * <br/>
