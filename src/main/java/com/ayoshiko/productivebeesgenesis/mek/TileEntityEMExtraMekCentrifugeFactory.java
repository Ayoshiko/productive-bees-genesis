 ...
 * EME扩展版MEK离心机工厂方块实体
 * <br/>
 * 继承EvolvedMekanismExtras的TileEntityEMExtraItemStackToItemStackFactory，
 * 复用EME的多进程并行处理、排序、升级体系（支持ABSOLUTE_OVERCLOCKED/SUPREME_QUANTUM/COSMIC_DENSE/INFINITE_MULTIVERSAL等级）。
 * <p>
 * 双配方处理路径（与ME版一致）：
 * <ul>
...
public class TileEntityEMExtraMekCentrifugeFactory extends TileEntityEMExtraItemStackToItemStackFactory
	...
	public TileEntityEMExtraMekCentrifugeFactory(Holder<Block> blockProvider, BlockPos pos, BlockState state) {
		...
		// EME基类提供getEnergySlot()，副输出槽2注册、IO配置、流体侧面配置、FLUID弹出器由Helper统一处理
		ejectorComponent = MekCentrifugeFactoryHelper.setupTertiarySlotsAndIO(
				...
				tier.processes, getEnergySlot(), energyContainer, fluidOutputTank,
	 ...
	 * <br/>
	 * 使用EME的EMExtraFactoryInputInventorySlot和EMExtraFactoryOutputInventorySlot。
	 * 主输出槽(y=57) + 副输出槽1(y=77) + 副输出槽2(y=97)。
	 * ProcessInfo的secondaryOutputSlot设为副输出槽1，副输出槽2用单独数组管理。
	 * 布局参数：baseX=27, baseXMult=19（与EME原版一致）。
	 */
	@Override
	 ...
	 * 配置卡兼容性检查
	 * <br/>
	 * EME工厂方块同时有AttributeFactoryType和EMExtraAttributeFactoryType属性，
	 * MekanismUtils.isSameTypeFactory()无法识别EME方块，导致配置卡粘贴失败。
	 * 此方法增加对AttributeFactoryType和EMExtraAttributeFactoryType的兼容检查，
	 * 允许同类型（如SMELTING）的EME/ME/原版工厂跨等级粘贴配置。
	 */
	@Override
	...
	/**
	 * 重写inputProducesOutput — 支持PB配方输出兼容性检查（带缓存）
	 * <br/>
	 * 父类只检查SMELTING配方，PB物品在输出槽非空时被阻止分配到空进程（sortInventory门控）。
	 * 重写后：SMELTING检查失败时回退到PB配方检查，验证PB配方输出与现有输出槽内容兼容。
	 * <p>
	 * SFM/AE2 高频调用此方法，增加 20 tick 缓存避免反复查配方和 hashItemAndComponents。
	 */
	@Override
	...
	public MachineUpgradeData getUpgradeData(HolderLookup.Provider provider) {
		return new MachineUpgradeData(provider, redstone, getControlType(),
				getEnergyContainer(), progress, getEnergySlot(), inputSlots, outputSlots,
