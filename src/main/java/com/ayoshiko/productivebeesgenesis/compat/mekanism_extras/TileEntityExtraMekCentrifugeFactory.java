...
public class TileEntityExtraMekCentrifugeFactory extends TileEntityExtraItemStackToItemStackFactory
	 ...
	 * 导致 {@code multiFluidDelegate::setFluidOutputTank} 抛 NPE。通过 {@link #getOrCreateDelegate()}
	...
	public void writeSustainedData(@NotNull HolderLookup.Provider provider, @NotNull CompoundTag data) {
		...
		// ME 的 TileEntityExtraFactory 不继承 Mekanism 的 TileEntityFactory，必须用 ME 专用 Accessor
	 ...
	 * <br/>
	 * 修复 SORTING 字段丢失：父类（TileEntityExtraItemStackToItemStackFactory）的
	...
	public void addContainerTrackers(MekanismContainer container) {
		...
		container.track(SyncableInt.create(getOrCreateDelegate()::getFluidOutputTankCount, getOrCreateDelegate()::setFluidOutputTankCount));
		// Task 8: SyncableBoolean 同步多流体槽模式状态 — 确保客户端 Tab 显示与服务端一致
		container.track(SyncableBoolean.create(() -> getOrCreateDelegate().getFluidOutputHolder() instanceof MultiFluidTankHolder, getOrCreateDelegate()::setMultiFluidModeSynced));
		// Task 3: 诊断日志 — 记录总 DataSlot 数、TileEntity 类型、调用源(callerId 替代运行时堆栈)
		CentrifugeFactoryCommonLogic.logTrackersDiagnostic(container, this, "TileEntityExtraMekCentrifugeFactory#addContainerTrackers");
	}

	/** 持久化PB进度、PB升级、AE2节点、AE2 per-tile状态和多流体槽 */
	@Override
	public void saveAdditional(@NotNull CompoundTag nbt, @NotNull HolderLookup.Provider provider) { CentrifugeFactoryCommonLogic.saveAdditional(nbt, provider, pbProcessor, pbUpgradeDelegate, productivebeesgenesis$ae2LifecycleHandler, this, getOrCreateDelegate().getFluidOutputHolder(), () -> super.saveAdditional(nbt, provider)); }

	/** 保存自定义数据为NBT — 供扳手拆卸持久化使用（含多流体槽内容） */
	@Override
	@NotNull
	public CompoundTag saveCustomDataForItem(@NotNull HolderLookup.Provider provider) { return CentrifugeFactoryCommonLogic.saveCustomDataForItem(provider, pbProcessor, pbUpgradeDelegate, productivebeesgenesis$getAe2StateHolder(), getOrCreateDelegate().getFluidOutputHolder(), this::getType, this); }

	/** 加载PB进度、PB升级、AE2节点、AE2 per-tile状态和多流体槽 */
	@Override
	public void loadAdditional(@NotNull CompoundTag nbt, @NotNull HolderLookup.Provider provider) { CentrifugeFactoryCommonLogic.loadAdditional(nbt, provider, pbProcessor, pbUpgradeDelegate, productivebeesgenesis$ae2LifecycleHandler, this, getOrCreateDelegate().getFluidOutputHolder(), () -> super.loadAdditional(nbt, provider)); }
	...
	public CentrifugeUpgradeData getUpgradeData(HolderLookup.Provider provider) {
		EnergyInventorySlot energySlot = ((TileEntityExtraFactoryAccessor) this).productivebeesgenesis$getEnergySlot();
		return CentrifugeFactoryCommonLogic.getUpgradeData(provider, redstone, getControlType(), getEnergyContainer(), progress, energySlot, inputSlots, outputSlots, isSorting(), getComponents(), pbUpgradeDelegate, productivebeesgenesis$getAe2StateHolder(), getOrCreateDelegate().getFluidOutputHolder());
	}

	/** 应用升级数据 — 先委托父类恢复标准字段，再恢复PB升级、AE2设置和多流体槽（Task 5） */
	@Override
	public void parseUpgradeData(HolderLookup.Provider provider, @NotNull IUpgradeData upgradeData) { CentrifugeFactoryCommonLogic.parseUpgradeData(provider, upgradeData, pbUpgradeDelegate, productivebeesgenesis$getAe2StateHolder(), getOrCreateDelegate().getFluidOutputHolder(), data -> super.parseUpgradeData(provider, data)); }
	...
	@Override
	public IExtendedFluidTank fluidOutputTank() { return getOrCreateDelegate().fluidOutputTank(); }
	@Override
	...
	@Override
	public int getFluidTankCount() { return getOrCreateDelegate().getFluidTankCount(); }
	@Override
	public IExtendedFluidTank getFluidTank(int index) { return getOrCreateDelegate().getFluidTank(index); }
	@Override
	public List<IExtendedFluidTank> getFluidTanks() { return getOrCreateDelegate().getFluidTanks(); }
	@Override
	public boolean isMultiFluidMode() { return getOrCreateDelegate().isMultiFluidMode(); }
	@Override
	...
	@Override
	public boolean areAllFluidTanksFull() { return getOrCreateDelegate().areAllFluidTanksFull(); }
	@Override
	public boolean canAllocateNewFluidTank() { return getOrCreateDelegate().canAllocateNewFluidTank(); }
	@Override
	public void setOrphanedMultiFluidTanksNbt(@Nullable CompoundTag nbt) { getOrCreateDelegate().setOrphanedMultiFluidTanksNbt(nbt); }
	@Override
	public @Nullable CompoundTag getOrphanedMultiFluidTanksNbt() { return getOrCreateDelegate().getOrphanedMultiFluidTanksNbt(); }
	...
	@NotNull
	public IExtendedFluidTank getFluidOutputTank() { return getOrCreateDelegate().getFluidOutputTank(); }
