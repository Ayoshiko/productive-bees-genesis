 ...
 */
public class GeometryLoaderHell implements IGeometryLoader<GeometryLoaderHell.HellGeometry> {
	...
	public HellGeometry read(JsonObject modelContents, JsonDeserializationContext deserializationContext) throws JsonParseException {
		...
		List<ResourceLocation> maskTextures = getMasks(modelContents, "hell");
	...
	private List<ResourceLocation> getMasks(JsonObject modelContents, String type) {
		JsonObject hell = modelContents.getAsJsonObject(type);
		if (hell == null) {
			throw new JsonSyntaxException("Missing " + type + " object.");
		}
		List<ResourceLocation> maskTextures = new ArrayList<>();
		JsonElement maskElement = hell.get("mask");
		if (maskElement != null && maskElement.isJsonArray()) {
			...
			maskTextures.add(ResourceLocation.tryParse(GsonHelper.getAsString(hell, "mask")));
		}
		return maskTextures;
	}
