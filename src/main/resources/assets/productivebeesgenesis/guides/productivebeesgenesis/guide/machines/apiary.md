...
- **蜂笼输入槽**（蜜蜂槽左侧，红色边框）：这一格接受**三类物品**——
  - 空的或装有蜜蜂的**蜂笼**：把蜜蜂装进/取出机器；
  - **资源蜜蜂刷怪蛋**：直接入驻空槽，省去抓蜂步骤（整叠会按空槽数量批量放入）；
  - **带基因的蜂蜜小食**：触发自动喂食，见下文"自动基因喂食"章节。
...
| 自动喂 | 把带基因小食**放进蜂笼输入槽**，机器自己挑蜜蜂喂 | 批量改造整箱蜜蜂 |
...
- **含"蜜蜂种类（type）"的小食会被拒绝**：PB 原版对这类小食本来就不施加基因（手持喂也只会提示 invalid use），放进去只会白扣，所以机器不接受。
...
蜂笼输入槽是标准 Mekanism 输入槽，所以**管道、漏斗、AE2 都能往里塞小食**：

- 用 AE2 接口/输出总线定量供给带基因小食，就能"一边产蜜脾一边改造蜜蜂"。
- 小食和蜂笼可以共用这一格：机器每 tick 先判断是不是小食，是就喂食，不是才按蜂笼逻辑处理，两者不会互相干扰。
...
8. **小食放进去了但不消耗**：说明当前没有蜜蜂能从这份基因里获益——要么属性已经不比小食差，要么小食里全是纯度 0 或"蜜蜂种类"基因。
