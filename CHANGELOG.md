...
>
> v1.5.4 起，所有历史 Release 附带的 JAR 文件名已重新构建，与当时的版本号严格匹配。
> 历史 JAR 文件名保留为构建时名称；Git 标签与 GitHub Release 标题的开发版前缀见下方说明。

> **历史开发版本标识**（2026-08-16）
>
> 首个正式版发布前的 GitHub Release（原 `v1.0.0` 至 `v2.0.9-hotfix`）均为开发快照，
...
- **P3／D16b2b 自动离心与蜂箱串联**：基础离心机可按输入规则和双层保留自动取用网络蜜脾／蜜脾块，逐真实 tick 消耗共享 FE；异构升级、固定作业种子及物品／流体结果保持独立。新增权重轮转与匹配状态，缺电、暂停和核心重建不免费补产；目标库存规则暂拒绝自动接单，正式玩家补料／取出仍待后续。
...
- **石料蜂/木材蜂复制黑名单失效**：机械蜂箱的动态花源判定与产物抽样现在都应用 Productive Bees 的 `productivebees:dupe_blacklist`。Industrial Foregoing 水培床虽然属于 `minecraft:dirt`，但已被 PB 明确列入复制黑名单，因此不再被石料蜂当作花源或产出；木材蜂同步采用相同规则。
- **特殊蜜蜂刷怪蛋无法直接入驻**：刷怪蛋右键空蜜蜂槽和蜂笼输入槽批量装入现支持 Productive Bees 注册的全部蜜蜂刷怪蛋，包括石料蜂、木材蜂等专用实体蜂；可配置资源蜂仍校验数据包蜂种。服务端按刷怪蛋注册类型创建临时 `ProductiveBee`，复用 PB 蜂笼序列化保存完整属性，并拒绝被 `ENTITY_DATA.id` 篡改的实体类型。
- **特殊蜜蜂路径一致性**：花源门禁与动态产物使用同一策略，禁用喂食槽不会参与判断；经 PB 13.13.5 实现核对，复制方块的特殊分支仅木材蜂与石料蜂需要黑名单，染料蜂和独立处理的 Wanna Bee 不受影响。

### 性能

- **PB 离心配方缓存跨机器共享**：将每台机器各自持有的 256 条 LRU 改为全局有界共享缓存，正结果与“无配方”负结果均可被所有离心机复用；保留每台机器的短期输入缓存，减少高并行工厂的重复配方遍历和内存占用。
- **缓存正确性与生命周期加固**：通用缓存键保留完整物品组件补丁并以哈希加速查找，避免哈希碰撞误命中；标签/配方重载、延迟配方重建及服务器停止时统一失效，防止旧 `RecipeHolder` 跨重载或跨存档残留。
...
- **精华转化升级**：蜂箱与离心机可将具有明确纯同物压缩配方的低级精华、粒和碎片按配方比例转化，每台机器最多安装一个。一般只转换压缩链最下级的一跳，保留不足一组的余数；混合原料、粗矿、锭、宝石及存储块不参与转换，多候选时优先使用明确的正向压缩配方。红石精华可按 8:12 合成红石粉，黑曜石碎片可合成黑曜石；下界之星精华可在同一批次连续经过碎片合成下界之星。配方索引构建改用标签候选交集，转换批次复用一次不可变索引快照，减少大型整合包的临时对象和重复缓存检查。同步提供物品、合成配方、解锁进度、贴图和中英文升级说明。
...
- **服务端配置拆分为三个文件**：原来的 `productivebeesgenesis-server.toml` 拆成 `productivebeesgenesis-gameplay-server.toml`（蜜蜂属性、获取、转化、产出、平衡规则、高级蜂箱）、`productivebeesgenesis-machines-server.toml`（离心机与机械蜂箱的基础参数、弹出策略、AE2 集成、升级上限、熔炼兼容、外部物流互操作）和 `productivebeesgenesis-capacities-server.toml`（容量矩阵）。原因：单文件已超过 24,000 字节，配置界面一屏塞不下，玩法与机器参数混在一起难以分发；拆分后整合包作者可以只覆盖需要的那一份。
- **旧配置自动迁移**：首次用新版本进入存档时，旧的单文件会被逐键复制进三个新文件，然后改名为 `productivebeesgenesis-server.toml.migrated.bak` 保留，不会删除。迁移是事务式的：先写临时文件并通过配置规格校验，再原子替换，任一步失败就整体回滚并保留旧文件等下次重试；中途崩溃时下次启动会自动恢复迁移前的状态。存档 `serverconfig/` 下的旧文件优先于 `config/` 下的旧文件。如果新的分文件已经被手动改过，就保留这些改动并跳过对应文件的迁移，只迁移仍是默认值的那些。
...
- **副产物销毁升级支持蜜蜡**：安装升级后，Productive Bees 原版/热能离心机和本模组 Mekanism 工厂会过滤 `c:waxes` 标签中的蜜蜡产出，同时继续销毁资源蜜蜂蜂蜜流体；其它物品与流体产物不受影响。
...
- **产物直通相邻容器**（新配置 `external_logistics.directContainerOutput`，默认开）：配方完成时先模拟目标容器能吃下多少，再直接写入相邻容器/管道，**跳过「先进输出槽、再被抽走」这一步**。原因：输出槽中转要付两次插入 + 两次抽取 + 两次槽位监听同步，还要等物流模组下一次轮询；而多数无线物流模组每 tick 只探测很少几个槽（例如天穹物流默认每面每 tick 只看 1 个槽，装满 8 张速度升级才 9 个），54 个输出槽的满配工厂靠对方来拉根本拉不干净。直通把产物在源头直接推出去，输出槽只作为「塞不下时的回落缓冲」。目标满、没有配置输出面、机器自动弹出关闭、或对面是通用机械逻辑运输管道（需要原版颜色/路由协议）时自动回落输出槽，不会丢物。

### 变更

- **容量矩阵改为按模组分组的数组**：`stack_multiplier` / `input_stack_multiplier` / `fluid_tank_multiplier` 与蜂箱 `stack_multiplier` 从每个等级一个键改为四组数组（通用机械、通用机械:扩展、进化通用机械、进化通用机械:扩展）。原因：原来 17 个等级 × 4 类 = 68 个独立键，每加一个兼容等级都要改四处；数组按固定索引对应等级，新增等级只改一处。旧文件里的每等级键值会在迁移时按索引写入对应数组，缺失的等级用当前默认值补齐。
- **容量矩阵配置界面禁止增删和移动元素**：数组索引就是等级含义，NeoForge 原生列表编辑器的上移/下移/删除会破坏映射，因此这些入口被移除，每一行直接显示等级名（basic、meAbsolute、emOverclocked 等）。关闭界面前会校验"容量必须随并行进程数不下降"，不满足就丢弃本次修改并保留打开前的值。
...
- **发布产物校验与体积报告**：构建时校验版本元数据、许可证、模组图标不超过 32 KiB 且不超过 256×256，以及不包含 datagen 类和已删除的重复资源，并输出体积明细报告。移除随功能增长已不适用的 1,800,000 字节 JAR 硬上限，保留资源与元数据检查；最终大小和 SHA-256 记录在 `PLATFORM_RELEASE.md`。
...
- **物品弹出改为自研高性能通道**：不再走通用机械的 `outputItems`。原版每个输出面每次只送走**一种**物品（成功插入第一种就返回），18 进程工厂同时产出十几种蜜脾时其余全部积压，输出满后整台机器停机——早期版本为此堆了十来个节流配置去缓解症状。现在一次遍历把所有输出槽、所有种类一次送完：先模拟目标能吃下多少、再精确取出插入（避免「取出后塞不下」的回填往返），轮转起点替代原版洗牌（同样避免靠后槽位饿死但零分配），某种物品被拒收后同一刻内不再为同类型槽位重复模拟。目标是通用机械逻辑运输管道时仍回退原版协议（需要颜色与路由信息）。
- **多流体槽对外轮转**：暴露给管道/物流模组的流体槽列表现在按游戏刻轮转起点（同一刻内索引稳定，跨刻旋转），固定启用。原因：物流模组抽流体时要么用 `drain(int)`（通用机械的实现会锁定**第一个非空罐**的流体类型），要么每 tick 只扫描前几个罐（如天穹物流每面每 tick 最多 8 个、热罐缓存硬编码 4 个）——两种行为都让主槽被反复清空、其余流体槽永远排不上队，最高等级工厂 18 个流体槽很快因积满而停机。轮转后每个槽都会周期性排到首位。内部访问（GUI 同步、NBT、配方写入）仍按原始顺序，槽位索引不受影响。
- **删除 11 项过时的弹出节流配置，行为固定为最大速度**：`mek_centrifuge.ejection.*` 与 `mek_apiary.ejection.*` 整段移除（弹出延迟、活动延迟、输出未变化跳过/跳过刻数、最大速度模式、最小间隔、高负载阈值/冷却、单 tick 最大弹出次数、阻塞阈值/冷却），`mek_centrifuge.basic.fluidEjectRate` 也一并删除、流体固定不限速。原因：这些参数全部是为了缓解通用机械弹出器「慢且贵」而存在的补丁，自研通道每刻只在真的有产物时跑一次 O(n) 遍历，再调慢只会降低吞吐；唯一还需要的「目标堵住时降频」已内置为自适应常量（连续 3 刻搬不动 → 退避 10 刻，产出内容一变立即恢复满速）。玩家更新后无需调参即可获得最大速度。
- **外部插入改为限制缓冲深度，不再限制车道**：工厂输入槽对外部自动化的准入策略从"每种物品每游戏刻只允许一个输入槽接收"改为"每个槽最多接收约 4 个真实刻的消耗量"（不改变槽位真实容量）。原因：异步规划型物流模组先在自己的快照上规划（快照里每个空槽都能收一组），再在主线程逐条提交，被我们拒绝就会出现"已提交 < 已规划"，从而触发对端最昂贵的同步补救路径（重新抓取整网络 + 全量同步搬运），每次操作都白烧一遍 MSPT；顺带这条策略从有状态（HashMap + 非原子的按刻判定）变成无状态纯函数，模拟与执行必然一致，也消除了被跨线程读取放大的竞态。机器内部转移（蜂箱直连、AE2 拉取）不受该策略约束，行为不变。
- **移除从未接线的 IO 限流配置**：`mek_centrifuge.io_limit.maxExtractPerTick` 与配套的 `RateLimitedItemHandler` 删除。原因：该键从注册以来没有任何代码读取，且「限制外部每刻拉取量」与本轮「让物流模组尽可能快取走产物」的目标相反。以上所有被删除的键在迁移时都按「已停用键」处理：值保留在备份文件中，不再写入新配置，也不会再报「未识别键」。
- **流体弹出速率与轮转不再读配置**：改为编译期常量（不限速 / 固定轮转），省掉每刻每槽的配置读取与缓存维护；外部物流配置分组只保留产物直通总开关，其余机器配置仍保留。
- **AE2 标签过滤覆盖全部输入**：标签表达式同时约束蜜脾和熔炼输入，不再要求先启用熔炼兼容；手动蜜脾条目、自动拉取与服务端配置校验采用一致的过滤规则。
- **标签选择器改为定高可搜索列表**：将原来的加减按钮长提示替换为六行滚动列表，支持搜索、勾选加入/移出表达式及白名单/黑名单切换；窗口高度随内容计算，修复候选标签过多时超出屏幕和搜索框焦点冲突。容量矩阵等级名称同步本地化。

### 修复

- **离心产物种类超过输出槽时停机**：为无法一次放下的多种产物规划有界延迟输出，输入仅扣除一次，剩余产物随机器保存；输入已耗尽或切换配方后仍继续排空已提交产物，避免缓冲滞留或被配方重置丢弃。大批量按延迟预算缩减，避免反复减半仍无法解决种类溢出。
- **加速熔炼的能量预算与扣费**：熔炼按实际操作数计算整批预算，不再叠加仅属于蜜脾生产的生产力并行倍率；批次账本统一累计和提交能耗，覆盖首 tick 新建配方缓存并在退出时清理线程作用域。批次容量跨 tick 保留，避免反复缩容裁掉 FE 电缆刚补充的能量；AE2 活动批次按预算缺口补能。
- **Mekanism 界面中的方块物品深度与拖拽渲染**：统一使用带实体上下文的槽位渲染入口，限定方块深度修正的渲染作用域，并覆盖 JEI 幽灵物品拖拽，减少蜜脾块图标的遮挡和渲染状态干扰。
- **高级蜂箱副产物销毁语义**：安装升级时抑制蜂蜜产物、花粉团及蜂蜜等级增长，保留其他产物；同步更新升级贴图与中英文说明。
- **模组图标体积**：`productivebeesgenesis.png` 从 126,291 字节压缩到 11,712 字节，减少无谓的 JAR 体积与内存占用。
- **弹出清单构建的 O(n²) 与每次洗牌**：通用机械构建弹出清单时会复制一份槽位列表、洗牌，再用 `indexOf` 反查下标。最高等级工厂有 18 进程 × 3 个物品输出槽 = 54 个槽，弹出延迟又被优化到 1 tick，于是每刻都要付一次列表分配 + 洗牌 + 54×54 次比较。现在改为轮转起点的 O(n) 零分配实现（轮转同样避免靠后槽位饿死，且相邻刻顺序稳定，对端缓存更友好），判定可弹出量的模拟调用与原版逐字一致。
- **第三方物流的同类型回填**：部分物流模组在目标插入后仍有剩余时，会调用源 handler 退回物品。输出槽现在接受「与槽内现有内容完全同类型」的外部回填，减少因输出槽仅允许内部插入而导致的退回失败。槽位为空时仍拒绝外部插入，因此不能保证覆盖第三方模组的所有回滚场景；流体侧的 `fill` 行为保持不变。
- **产物直通与逐刻弹出共用同一份目标解析**：输出面方向集合按「朝向 + 侧面配置」缓存，只在玩家转动机器或改侧面配置时重建；每个方向持有一个 `BlockCapabilityCache`，不再每刻做方块实体查找。同时不会周期性调用 `invalidateCapabilities`——那会连带清空对端物流模组的热槽/退避/拒收学习结果，让它退回冷启动。
- **重复的机械蜂箱 LED 模型**：删除 `factory/mek_apiary/front_led/` 下 33 个与离心机完全相同的模型文件，蜂箱模型改为直接引用 `factory/mekanism_centrifuge/front_led/` 的共享模型，外观不变。
- **失效的纹理引用**：离心机方块模型的西侧面引用了已不存在的 `honey_mixer_s2`，改为与东侧一致的 `honey_mixer_s1`；根成就背景引用了已删除的 `mekanism_centrifuge_front.png`，改为 `honey_mixer/honey_mixer_f.png`。同时删除 4 个未被任何模型引用的重复纹理和物品栏里用不到的 `item/infinitycreation_comb_block.png`（方块纹理仍在）。
- **服务端配置状态判断**：所有 `SERVER_SPEC.isLoaded()` 检查改为 `areServerSpecsLoaded()`，三份服务端配置必须全部就绪才认为可用，避免拆分后只加载了一份就去读另一份里的键。玩法过滤器同步包保存时只写玩法配置文件，不再整体落盘。
- **AE2 待处理缓冲登记判定顺序**：`canRegister` 先判断容量未满再线性查找，正常路径不再每次扫描最多 64 个条目。
- **本机无法处理的蜜脾不再反复空转拉取**：整合包里大量蜜脾只有蜂、没有本模组的离心配方（要靠其它模组的机器处理，例如 chemlib 元素蜜脾走氧化机/分解机）。这类物品按 Item 引用判定仍然"像蜜脾"，于是每轮都被拉进候选窗口、付一次"建栈 + 逐槽校验 + 配方查找"，却永远插不进槽，也因此从不触发按键退避 —— 表现就是"卡但 TPS 不掉"。现在候选分类阶段增加了本机可处理性闸门，判定委托机器已有的输入校验（自动覆盖万象创世动态配方、PB 离心配方、熔炼兼容三条语义），并按 `(Item, bee_type)` 而不是 `AEItemKey` 记忆化（spark vVh8WfPCN3 实测按 `AEItemKey` 记忆化时仅 map 查找自身就要 508 ms）。缓存每机器一份、上限 2048 条，配方/标签重载与熔炼兼容开关变化时整表失效；每轮拉取限速 64 次首次判定，避免配方重载后的首轮刷新把全量遍历堆在同一 tick。判定异常时按"可处理"放行，宁可多探测也不饿死合法蜜脾。被闸门拒绝的蜜脾直接终止，不会下落到熔炼分支 —— 否则整合包为 `c:honeycombs` 注册的熔炼配方会把它重新拉进来并产出错误结果。
- **槽位永久拒绝的物品进入按键退避**：容量规划阶段现在区分"槽位校验器明确拒绝"和"暂时没位置"两种情况。前者说明本机永远不接受该物品，是持久信号，据此记一次按键退避；原先这类键从不进入批量拉取路径，因此永远不会被退避，只能全速重复探测。这是上一条的兜底防线。
- **AE2 拉取配置里的蜜脾标记恢复齿轮与库存显示**：1.0.6 起蜜脾标记被改存成"按蜂种"的模糊条目，而标记上方的齿轮（单次拉取数量 / 库存保留量 / 无限拉取 / 库存模式）和下方的网络库存数量只对精确条目渲染，于是蜜脾标记退回成"只有一个图标"的旧样式，数量编辑器也点不开；从 JEI 拖蜜脾块进去还会显示成蜜脾 —— 模糊条目在非精确模式下把 `#block` 形态丢掉了。熔炼物品标记和 1.0.6 之前留下的旧标记不受影响，因为它们本来就是精确条目，这正是"旧设置仍然正常、新标记不正常"的分水岭。现在蜜脾与熔炼物品一样按精确指纹存储；"蜜脾与蜜脾块共用一份拉取配额"的模糊语义完全不变，由匹配层在非精确模式下按蜂种分组保证，不需要靠牺牲条目形态来实现。旧存档里 1.0.6~1.0.7 期间标记的蜜脾会在打开配置界面时就地升级为精确条目并落盘，不用手动重新拖一遍；模糊条目本身也不再丢 `#block`，关掉精确模式不会再把蜜脾块标记降级成蜜脾。升级时按蜂种构造的键还会用 ME 网络快照校正：网络里的同种蜜脾若带别的数据组件（被改过名等），以网络里真实存在的那一个键为准，否则下方"可见库存"会读成 0、输出槽也会指向一个网络里不存在的键。这条校正在非精确模式下持续生效（只换键，玩家调过的拉取量/保留量/无限/库存模式一律保留），精确模式与熔炼物品条目一律不动 —— 那里"键完全相等"本身就是玩家要表达的语义。
- **迁移不再触发全局配置重载**：只有"存档 `serverconfig/` 里原本没有拆分文件、NeoForge 已把配置绑定到全局 `config/`"这一种情况才重新加载服务端配置；整合包只改全局 `config/` 的常见情况直接替换内存对象。原因：`unloadConfigs` 是全局操作，会让所有模组（含 NeoForge 自身）多收一轮 Unloading/Loading 事件。重载过程也加了异常兜底，失败时本次会话继续使用迁移前的配置对象而不是开服失败。

### 性能

- **Wanna Bee 战利品采样共享预算**：同一蜂种组的不同生产力等级共享最多 16 次独立战利品采样，按各等级生产轮数分配权重；小批次仍逐次采样，大批次保留代表轮数总和，但稀有掉落的短期波动可能更明显。
- **机械蜂箱直连与界面缓存**：同刻复用离心机输入槽预扫描，直接写槽后主动失效；喂食槽禁用位掩码、有效花朵统计和客户端提示按内容版本缓存。蜂箱 GUI 拆分槽位几何、点击交互、渲染和分页职责，减少重复遍历。
- **染料蜜蜂产出不再每次遍历全部合成配方**：染料蜜蜂的产物要靠"花 → 单原料合成配方 → 染料"推断（复刻 PB 原版 `BeeHelper.getRecipeOutputFromInput`），而这条路径原先每次调用都跑一遍 `getAllRecipesFor(RecipeType.CRAFTING)` 全表遍历，还对每个单原料配方调 `Ingredient.getItems()`（含标签展开与栈数组构造）。更糟的是多花蜜蜂分支为了让产物随喂食槽内容实时变化，本来就绕过了产物缓存，所以整合包上万条合成配方会在每个产出周期被完整扫一遍。现在改为 `物品 → 配方产出` 的不可变索引，成本一次性前移到配方重载后的首次查询，之后是一次哈希查找。索引语义与原实现逐条对齐：同样"首个匹配配方胜出"、同样跳过空产出配方，因此产物结果不变。构建失败按 5 秒节流重试，不会因一次异常就让染料蜜蜂长期停产。
- **蜜脾类型组件只解析一次**：`bee_type` 数据组件类型此前在每个热路径调用点都走一次 `DeferredHolder.get()` 注册表查找，AE2 容量规划里单次 `matchesComponents` 就要查 4 次。spark AHlDkwd9n9 实测 `DeferredHolder.get` 自耗 884 ms、占服务端线程 1.47%，是全服第 2 热的方法（仅次于空闲等待）；NKn1ZLQN2W 为 468 ms / 0.78%。该组件类型在模组构造期注册一次后整个进程内恒定，不随数据包重载变化，因此改为一次性解析后复用，共覆盖 AE2 拉取、蜜脾识别、配方查找、输入校验、万象批量规划、离心机工厂与宇宙渲染共 10 处热路径；注册期构造默认组件的路径保留原调用形式（此时 `DeferredHolder` 尚未绑定）。
- **AE2 指纹缓存改 LRU，不再满即整表清空**：每台机器的 `AEItemKey → SNBT 指纹` 缓存上限 128 条，原先一满就整表 `clear()`。物品种类超过上限的大网络会周期性丢弃全部热条目，清空后每个键都要重新跑一遍 Codec 编码加字符串遍历 —— 正是这个缓存要消除的开销。现在按访问顺序淘汰最久未使用的一条，稳定复用的键始终驻留。
- **输入校验缓存覆盖满进程数**：`InputValidationCache` 的槽位从 4 条提到 20 条，覆盖最高等级工厂的 19 个并行进程。此前多进程工厂各进程放不同蜜脾时会互相驱逐，缓存退化成每次探测都重跑一遍熔炼加 PB 配方查找；未命中时多比较的那十几次指纹（物品引用 + 可空蜂种 ID + 整数）远比一次配方查找便宜。
- **蜜蜂产物归属缓存加上限，且不在持锁时查配方**：GUI 排序/搜索用的 `蜂种 → 产物模组` 缓存原为无上限映射，现在限 512 条并按最久未使用淘汰，作为自定义数据包生成大量一次性蜂种时的内存防线。同时把 `computeIfAbsent` 改为"先查缓存、未命中在锁外解析、再写回"，避免持锁期间执行配方查找；并发争用时最多重复算一次（结果幂等），同一蜂种最终只保留一份实例。
- **AE2 输入拉取的七处热点一并处理**（依据玩家提交的 spark BkTP3d9oSc：398 个模组、TPS 12.13、开着时间加速；本模组占服务端主线程 5.88%，其中拉取路径 4.55%）：
  - **抽取前不再无条件编码 SNBT 指纹**。指纹原本只为一个判断服务 ——「待处理缓冲的类型表还有没有位置」。而这个缓冲在正常情况下是空的，位置永远有，指纹算了就扔。现在先做一次 O(1) 的空位判断，只有 64 种类型全部积压（异常态）才编码指纹去精确查重；真正要登记剩余物时才算指纹。该报告中这次纯浪费的编码占 372 ms（0.62%，其中 Mojang Codec 编码 244 ms、SNBT 字符串化 124 ms）。抽取之后取指纹改走绝不抛异常的兜底编码：抽取已不可撤回，此刻抛异常就等于丢物品。
  - **保留下限的实时校验按游戏刻记忆**。配置了库存保留量时，每次抽取前都要做一次真实的模拟抽取来确认不会越线，而这次模拟会穿透网络上每一个存储元件（万象元件、大宗盘的压缩链、无限存储的存档数据）。时间加速模组会在同一个游戏刻里把机器 tick 很多次，于是同一个键在同刻被反复实时探测，而这些重复探测的信息量为零 —— 本机自己的消耗由同刻视图精确扣减，别的设备的消耗在原实现里也只能在两次抽取之间的窗口被看见。现在每个游戏刻保留一次真实探测，其余复用扣减后的结果，安全语义与「每游戏刻拉取一次」完全等价。该报告中这条链路占 612 ms（1.02%）。
  - **候选判定缓存改按物品身份记忆**。熔炼配方判定、标签过滤判定原先都以完整的 `AEItemKey` 为缓存键，而 `AEItemKey.equals` 内部走 `ItemStack.isSameItemSameComponents`（装了 geckolib 还会被它的 mixin 包一层），该报告中这个方法自耗 1416 ms（2.36%），是全服第 5 热的方法。熔炼判定改为「没有数据组件补丁的物品按物品引用记忆，有补丁的才退回完整键」，兼顾正确性与命中率；标签判定的输入本来就只有物品本身，直接改为按物品引用记忆。这与蜜脾可处理性缓存早已采用的做法一致，本次是补上最后两处遗漏。
  - **Mekanism 熔炼查询句柄不再每次重新解析**。每次缓存未命中都要穿一层注册表解析（`DeferredHolder.value()`）才能拿到查询入口，该报告中它自耗 320 ms（0.53%），几乎与真正的配方查询等量。句柄在一次配方重载周期内恒定，现在随配方版本一起缓存，配方重载时一并换新（继续持有旧句柄会按过期配方表作答）。
  - **候选分类结果随条目传递，排序阶段不再重新分类**。排序需要知道每个候选是蜜脾还是熔炼输入，原先为此重新跑一遍分类判定，等于每次拉取额外穿过一轮配方缓存与标签缓存。分类结果只由候选身份决定，与排序无关，现在在准入阶段就写进条目一路带下去。
  - **槽位准入每轮只判定一次**。「这个槽位收不收这个物品」的判定原先按「候选类型 × 槽位」调用，而 Mekanism 工厂的全部输入槽由同一循环用同一台机器的同一个方法创建，语义上要么都收要么都不收。现在每轮按物品判定一次并以槽位实现类为守卫（遇到第三方自定义槽立刻重新判定），该报告中这条重复占 236 ms。
  - **候选去重改哈希集合**。候选列表在大型网络可达数千项，而选中上限是「进程数 × 2」，原先的线性去重使整轮扫描付出 O(候选数 × 选中上限) 次物品键比较，正是上面 `AEItemKey.equals` 的另一个来源。
- **三处逐候选布尔判定缓存统一换成无锁分代表**：熔炼配方判定、标签过滤判定、蜜脾可处理性判定原先都是「synchronized + 访问顺序 LinkedHashMap」。这个组合在每刻数千次的判定路径上有三重固定成本：每次读写都进出监视器（哪怕只有 tick 线程真正竞争）、命中也要重排双向链表节点、值是装箱的布尔对象。新容器写满一代后把整代降为上一代并复用旧数组，查询顺序为当前代 → 上一代，命中上一代即提升回来：淘汰是 O(1) 的整代丢弃，热键能跨越任意多次降代继续驻留，不会出现「满即整表清空」导致的命中率塌陷，驻留上界仍是单代上限的两倍。网格回调要求失效时只递增一个计数器，真正清表推迟到 tick 线程下一次查询，因此热路径上再没有任何锁。该报告中仅熔炼判定一处的自耗就有 344 ms（0.57%），其中大部分属于上述固定成本而非真正的配方查询。

### 测试

- **本轮机器与升级回归**：新增批次能耗账本、容量预算、延迟批量边界、精华压缩规则、粗矿换算、喂食槽禁用和蜂箱槽位几何测试；补充直通输出、产物溢出、加速熔炼、蜂箱直连缓存及 AE2 蜜脾条目的源码接线断言。接线断言用于检查调用关系，不替代游戏内集成测试。
- **迁移回归**：覆盖 68 个等级键全部迁移、非法值回落默认值而不影响同组其他值、无平衡性配置的老存档兼容、部分文件已被改过时的部分迁移、可选集成键始终存在、事务提交与回滚、崩溃恢复、备份不被覆盖、存档域优先于根配置。
- **备份轮转与叠加迁移**：覆盖备份槽位轮转（`.migrated.bak` → `.migrated-2.bak` → …）、整合包重复分发同一份文件按内容识别、整合包新值叠加时保留玩家改过的键、上一次备份不可解析时退化为只补齐默认值键。
...
- **有界 LRU 淘汰语义**：新增 `BoundedLruMap` 的真实单测（非源码断言），覆盖上限始终不被突破、淘汰的是最久未使用而非最早插入、热键在持续冷键写入下常驻、重写已有键也刷新其新近度、非正上限抛异常、四线程并发写仍守上限、`putIfAbsent` 收敛到同一实例。这是"满即整表清空 → LRU"这一改动的正确性依据。
- **配方查找与组件解析接线**：钉住染料查找走索引且不再直接全量遍历、索引以不可变快照发布并保留"首个匹配胜出 / 跳过空产出"语义、配方重载时失效、输入校验缓存槽位覆盖满进程数、产物归属缓存有界且昂贵解析在锁外执行；组件侧钉住 10 处热路径不得回退到 `DeferredHolder.get()`、`matchesComponents` 单次调用只解析一次组件类型、解析器惰性求值且不设失效入口（静态注册表恒定）、注册期路径保留原调用形式。这些改动被改回原样时行为完全正确、只是开销回涨，纯逻辑测不出来。
- **无锁分代记忆表语义**：新增 `BoundedBooleanMemo` 的真实单测，覆盖三态语义（未记录 / 记录为 false 必须可区分）、上一代命中被提升回当前代且只有最冷的一代被整体丢弃、驻留数恒不超过单代上限的两倍、`requestClear` 在下一次访问时生效且不影响之后的新写入、其它线程投递的失效请求必须被 tick 线程观察到（AE2 网格回调场景）、`clearNow` 立即清空两代、null 键按未记录处理不污染表、非正上限抛异常。这是「同步 LRU → 无锁分代表」这一改动的正确性依据。
- **AE2 拉取热点接线（本轮）**：钉住三处布尔判定缓存都用分代表且热路径无 `synchronized`、熔炼判定按「有无组件补丁」分档且 Mekanism 查询句柄随配方版本缓存、标签判定按物品引用记忆、保留探测走按刻记忆化入口且提交抽取后同步扣减两张表、上限截断判定（只有「上限不小于本次」或「未被截断」才可复用）、分类结果随条目传递且排序阶段不再调用分类、槽位准入以槽位实现类为守卫按轮记忆、游标扫描用哈希去重且去重集合跨 tick 复用、抽取前只做 O(1) 空位判断而抽取后取指纹走绝不抛异常的兜底编码。这些接线被改回原样后行为依旧正确，只是 MSPT 悄悄涨回去。

### English

#### Added

- **Essence Conversion upgrade**: hives and centrifuges can convert low-tier essence, nuggets and fragments using unambiguous same-item compression recipes, with a limit of one upgrade per machine. Incomplete groups are preserved, explicit compression recipes win ambiguous unified-material candidates, and mixed ingredients, raw ores, ingots, gems and storage blocks remain excluded. Redstone essence uses its 8:12 redstone recipe, obsidian shards can form obsidian, and nether-star essence can continue through its shard intermediate to a nether star in one batch. Tag candidates now use set intersection and each conversion batch reuses one immutable index snapshot to reduce temporary allocations and repeated cache checks in large modpacks. Includes recipes, unlock advancements, textures and bilingual tooltips.
...
- **Server config split into three files**: the former `productivebeesgenesis-server.toml` is now `productivebeesgenesis-gameplay-server.toml` (bee attributes, acquisition, conversion, produce, balance rules, advanced beehive), `productivebeesgenesis-machines-server.toml` (centrifuge and apiary basics, ejection, IO limits, AE2 integration, upgrade caps, smelting compat) and `productivebeesgenesis-capacities-server.toml` (capacity matrices). The single file had grown past 24,000 bytes, mixed gameplay with machine tuning, and no longer fit one config screen; modpack authors can now ship only the part they changed.
- **Automatic migration of the old config**: on the first launch with this version the old single file is copied key by key into the three new files and then kept as `productivebeesgenesis-server.toml.migrated.bak` — it is never deleted. The migration is transactional: staged files are written and validated against the config spec before an atomic replace, any failure rolls everything back and leaves the old file for the next attempt, and an interrupted migration is recovered on the next launch. An old file under the world's `serverconfig/` wins over one in `config/`. Split files that were already edited by hand are kept and skipped; only files still at their defaults are migrated.
...
- **Byproduct Destruction upgrade now discards wax**: when installed, Productive Bees' base/heated centrifuges and this addon's Mekanism factories filter item outputs in the `c:waxes` tag, while continuing to discard Productive Bees honey fluid. Other item and fluid outputs are preserved.
...
- **Faster external item and fluid transport**: the item ejector visits all eligible output slots with a rotating start and bounded rejection memo instead of stopping after the first accepted item type. Blocked output backs off adaptively, while logistical transporters use Mekanism's native protocol. External fluid tank views rotate each tick without changing internal GUI/NBT ordering.
- **Obsolete logistics throttles removed**: remove the old apiary/centrifuge ejection sections, configurable fluid ejection rate and unused external extraction limit. Migration recognizes retired keys and keeps their old values in backups. External insertion now limits each input slot to roughly four real ticks of consumption instead of admitting one lane per item type per tick; internal transfers retain their existing admission path.
- **AE2 tag filtering applies to all inputs**: expressions constrain both combs and smelting inputs and no longer require smelting compatibility to be enabled. Manual comb entries, automatic pulling and server-side validation use the same filtering rules.
- **Searchable fixed-height tag picker**: replace oversized add/remove tooltips with a six-row scrollable list, search and click-to-toggle entries. Keep whitelist/blacklist selection, size the window from its content and fix nested search-field focus. Capacity matrix tier names are localized as well.
- **Capacity matrices are now per-mod arrays**: `stack_multiplier`, `input_stack_multiplier`, `fluid_tank_multiplier` and the apiary `stack_multiplier` moved from one key per tier to four arrays (Mekanism, Mekanism Extras, Evolved Mekanism, Evolved Mekanism Extras). The old layout meant 17 tiers × 4 matrices = 68 separate keys and four edits per new compatibility tier; array indices now carry the tier meaning. Legacy per-tier values are written into the matching array index during migration, and tiers absent from the old file fall back to the current defaults.
- **The capacity matrix screen no longer allows adding, deleting or reordering entries**: the array index *is* the tier, so NeoForge's list editor controls that would break the mapping are hidden and every row is labelled with its tier key (basic, meAbsolute, emOverclocked, …). Closing the screen validates that capacity does not decrease relative to parallel process count; a violating edit is discarded and the values from before opening are kept.
- **Factory multipliers are now a per-session snapshot**: the three centrifuge matrices and the apiary output matrix are read once into an immutable snapshot when the config loads, and the runtime only reads that snapshot. Previously every `getLimit` call went through a config read, which Spark measured at 5.33% CPU under 256× acceleration, and changing a multiplier mid-session could make existing slot capacities jump. The tradeoff is that capacity matrix edits now require a restart (stated in the screen). An invalid ordering only falls back to that group's defaults for the current run; the config file itself is left untouched.
- **Three separate config screens**: the mod list entry now opens gameplay, machines and capacity matrices separately; the gameplay screen keeps the existing filter and balance-preset custom screens.
- **Data generation code is out of the shipped artifact**: `datagen` moved to its own source set that is only compiled and run during development, so the released JAR no longer contains `com/ayoshiko/productivebeesgenesis/datagen/` classes. Recipe ids (`productivebeesgenesis:mek_apiary`, `productivebeesgenesis:mek_centrifuge`, …) and generated output are unchanged.
- **Release artifact verification and size reporting**: the build checks version metadata, licenses, an icon no larger than 32 KiB and 256×256, and the absence of datagen classes and removed duplicate resources. The obsolete 1,800,000-byte JAR cap is removed as functionality grows; resource checks and the size report remain. Final artifact size and SHA-256 are recorded in `PLATFORM_RELEASE.md`.
- **Dead balance compatibility layer removed**: `BalanceConfigCompatibility`'s legacy logic was folded into the new migration service, so the class and its test are gone instead of keeping two equivalent implementations. Three low-value tests that only asserted constants or source strings (`BalanceConfigCompatibilityTest`, `Ae2PerTileStateNbtCodecTest`, `UselessByproductUpgradeHelperTest`) and the environment-variable-gated `externalMigrationMatchesGeneratedFilesWhenRequested` were removed as well; the latter's coverage is now provided by the two modpack regression tests.

#### Fixed

- **More output types than centrifuge slots**: bounded deferred output lets multi-product recipes commit their input once and drain remaining products later. Committed output is persisted and continues draining after the input becomes empty or changes recipe. Batch sizing uses the deferral budget instead of repeatedly halving a batch that still has too many output types.
- **Accelerated smelting energy accounting**: smelting budgets follow actual operations without applying the honeycomb-only productivity parallel multiplier. A batch ledger combines energy charges, covers recipe caches created during the first tick and cleans up its thread scope. Batch capacity survives between ticks so ordinary FE cables can refill it without repeated capacity clamping; active AE2 batches request their budget shortfall.
- **Block-item depth and drag rendering in Mekanism screens**: use the standard entity-aware slot renderer and a scoped depth adjustment, including JEI ghost ingredient dragging, to reduce comb-block occlusion and render-state interference.
- **Advanced-hive byproduct destruction**: suppress honey products, pollen puffs and honey-level growth while preserving other products; update the upgrade texture and bilingual descriptions.
- **External logistics rollback and target lookup**: occupied output slots accept matching-item returns from external handlers, reducing rollback failures; empty slots still reject insertion. Direct output and ejection share cached output-face and capability lookup, avoiding periodic capability invalidation that would reset neighboring logistics caches.
- **Mod icon size**: `productivebeesgenesis.png` shrank from 126,291 to 11,712 bytes.
- **Duplicated apiary LED models**: the 33 files under `factory/mek_apiary/front_led/` were byte-identical to the centrifuge ones and were removed; apiary models now reference the shared `factory/mekanism_centrifuge/front_led/` models. Appearance is unchanged.
- **Dangling texture references**: the centrifuge block model referenced the missing `honey_mixer_s2` on its west face and now uses `honey_mixer_s1` like the east face; the root advancement referenced the deleted `mekanism_centrifuge_front.png` and now uses `honey_mixer/honey_mixer_f.png`. Four unreferenced duplicate textures and the unused `item/infinitycreation_comb_block.png` were removed (the block texture remains).
- **Server config readiness checks**: every `SERVER_SPEC.isLoaded()` check became `areServerSpecsLoaded()`, so all three server configs must be loaded before any of their keys are read. Saving the gameplay filter sync payload now writes only the gameplay file instead of all three.
- **AE2 pending buffer registration order**: `canRegister` checks remaining capacity before scanning, so the normal path no longer walks up to 64 entries every call.
- **Combs this machine cannot process no longer spin in the pull loop**: modpacks contain many combs that have a bee but no centrifuge recipe in this mod (they belong to another mod's machine — chemlib element combs go through the oxidizer/decomposer, for example). Such items still look like combs when judged by item reference, so every round they entered the candidate window and paid a "build stack + per-slot validator + recipe lookup" cost while never fitting into a slot — and therefore never tripped the per-key backoff either, which showed up as "laggy but TPS is fine". Candidate classification now has a machine-capability gate that delegates to the machine's existing input validation (covering Myriad dynamic recipes, PB centrifuge recipes and smelting compat in one shot) and memoises by `(Item, bee_type)` instead of `AEItemKey` — spark vVh8WfPCN3 measured 508 ms of self time in map lookups alone when keyed by `AEItemKey`. The cache is per machine, bounded at 2048 entries, and invalidated wholesale on recipe/tag reload or a smelting-compat toggle; each pull window allows at most 64 first-time probes so the first refresh after a recipe reload does not pile every full scan into one tick. A probe failure allows the item through — over-probing is far cheaper than starving a legitimate comb. A comb rejected by the gate stops there and never falls through to the smelting branch, because a modpack smelting recipe registered for `c:honeycombs` would otherwise pull it back in and produce the wrong output.
- **Items a slot permanently rejects now enter per-key backoff**: capacity planning distinguishes "the slot validator refused this item" from "no room right now". The former means this machine will never accept the item, which is a durable signal worth a backoff entry; previously such keys never reached the batch pull path, so they were never backed off and were probed at full speed forever. This is the safety net behind the previous entry.
- **Comb marks in the AE2 pull config get their gear and stock readout back**: since 1.0.6 a comb mark was stored as a fuzzy per-bee-type entry, but the gear above a mark (per-pull amount / stock floor / unlimited pull / stock mode) and the network stock count below it are only rendered for exact entries, so comb marks fell back to the old "icon only" look and the amount editor would not open. Dragging a comb block in from JEI also showed a plain comb, because a fuzzy entry dropped the `#block` form outside precise mode. Smelting-item marks and marks made before 1.0.6 were unaffected since they already were exact entries — which is exactly why "the old settings still work but new marks do not". Combs are now stored as exact fingerprints just like smelting items; the fuzzy behaviour of "a comb and its comb block share one pull quota" is unchanged and is guaranteed by the matcher grouping by bee type outside precise mode, instead of by degrading the entry shape. Comb marks made with 1.0.6/1.0.7 are upgraded in place and saved the next time the config window is opened, so nothing has to be re-dragged, and fuzzy entries no longer lose `#block`, so turning precise mode off no longer downgrades a comb block mark to a comb. The key an upgrade builds from a bee type is also corrected against the ME network snapshot: if the network's comb of that species carries extra data components (a renamed comb, for instance), the key that actually exists in the network wins — otherwise the stock readout below would show 0 and the output slot would point at a key the network does not have. That correction keeps running outside precise mode (it only swaps the key; the per-slot amount, stock floor, unlimited and stock-mode settings are preserved), while precise mode and smelting-item entries are never touched, because there "the key matches exactly" is the whole point.
- **Migration no longer forces a global config reload**: the server configs are reloaded only when the world's `serverconfig/` had no split files yet and NeoForge therefore bound them to the global `config/` directory; the common modpack case of editing only the global `config/` swaps the in-memory objects directly. `unloadConfigs` is a global operation that makes every mod (including NeoForge itself) receive an extra Unloading/Loading round. The reload is also guarded, so a failure keeps the pre-migration config objects for the session instead of failing server start.

#### Performance

- **Shared Wanna Bee loot sampling budget**: productivity levels in the same bee-type group share at most 16 independent loot samples, weighted by their production rounds. Small batches retain exact per-round sampling; large batches preserve total represented rounds but can show greater short-term variance in rare drops.
- **Apiary transfer and UI caches**: reuse centrifuge input scans within a tick and invalidate after bypass writes. Cache feeder masks, flower statistics and tooltips by content version. Split apiary GUI geometry, interaction, visuals and pagination into focused helpers.
- **Dye bee output no longer walks every crafting recipe**: a dye bee's produce is inferred through "flower → single-ingredient crafting recipe → dye" (mirroring PB's `BeeHelper.getRecipeOutputFromInput`), and that path used to run a full `getAllRecipesFor(RecipeType.CRAFTING)` scan on every call, calling `Ingredient.getItems()` (tag expansion plus stack array construction) for each single-ingredient recipe. Worse, the multi-flower branch deliberately bypasses the produce cache so the output can follow the feeder contents, so a modpack's tens of thousands of crafting recipes were scanned in full on every production cycle. It is now an immutable `item → recipe result` index: the cost moves once to the first query after a recipe reload, and every later call is a hash lookup. The index matches the original semantics line for line — first matching recipe wins, empty results are skipped — so produce is unchanged. A failed build is retried on a 5-second throttle so one exception cannot stall dye bees indefinitely.
- **The comb type component is resolved once**: the `bee_type` data component type used to go through a `DeferredHolder.get()` registry lookup at every hot-path call site — four times in a single `matchesComponents` during AE2 capacity planning. spark AHlDkwd9n9 measured 884 ms of self time in `DeferredHolder.get`, 1.47% of the server thread and the second hottest method overall (behind only idle waiting); NKn1ZLQN2W showed 468 ms / 0.78%. The component type is registered once during mod construction and stays constant for the process — it does not change across datapack reloads — so it is now resolved once and reused across ten hot paths (AE2 pulling, comb identification, recipe lookup, input validation, Myriad batch planning, the centrifuge factories and cosmic rendering). Registration-time paths that build default components keep the original call form, since the `DeferredHolder` is not bound yet at that point.
- **The AE2 fingerprint cache uses LRU instead of clearing wholesale**: each machine's `AEItemKey → SNBT fingerprint` cache is bounded at 128 entries and used to `clear()` the whole table once full. On a large network with more item types than that, this periodically dropped every hot entry, and each key then had to re-run the Codec encode plus string walk — exactly the cost the cache exists to remove. It now evicts the least recently used entry, so steadily reused keys stay resident.
- **The input validation cache covers the full process count**: `InputValidationCache` went from 4 slots to 20, covering the 19 parallel processes of the highest factory tier. Previously a multi-process factory holding a different comb per process evicted its own entries, degrading the cache into a fresh smelting-plus-PB recipe lookup on every probe. The extra dozen-odd fingerprint comparisons on a miss (item reference, nullable bee-type id, int) are far cheaper than one recipe lookup.
- **The bee product-ownership cache is bounded and no longer holds a lock while looking up recipes**: the `bee type → product mod` cache used by GUI sorting and search was an unbounded map; it is now capped at 512 entries with least-recently-used eviction as a memory guard against datapacks that generate many one-off bee types. `computeIfAbsent` also became "read the cache, resolve outside the lock on a miss, then write back", so recipe lookups no longer run while holding the table lock. Under contention the resolve may repeat once (the result is idempotent) and one instance per bee type is retained.
- **Seven AE2 input-pull hot spots addressed together** (from a player-submitted spark report, BkTP3d9oSc: 398 mods, TPS 12.13, time acceleration active; this mod accounted for 5.88% of the server thread, 4.55% of it in the pull path):
  - **No more unconditional SNBT fingerprint encoding before an extract.** The fingerprint served exactly one question — "does the pending buffer's type table still have room?" — and that buffer is empty in normal operation, so the answer is always yes and the fingerprint is computed and thrown away. There is now an O(1) free-slot check first; only when all 64 type slots are backlogged (an abnormal state) is a fingerprint encoded for an exact lookup, and the fingerprint for a leftover registration is computed only when a leftover actually has to be registered. In that report this purely wasted encoding cost 372 ms (0.62%), of which 244 ms was the Mojang Codec encode and 124 ms the SNBT stringification. Taking a fingerprint *after* an extract now uses an encoder that cannot throw: the extract is already irreversible, so an exception there would mean losing the items.
  - **The reserve-floor recheck is memoised per game tick.** With a stock reserve configured, every extract is preceded by a real simulated extract to confirm the floor will not be crossed, and that simulation passes through every storage cell on the network (universal cells, a bulk cell's compression chain, an infinite storage's saved data). Time-acceleration mods tick the machine many times within one game tick, so the same key was probed live over and over — with zero information gained, because this machine's own consumption is deducted precisely from the same-tick view, and other devices' consumption could only ever be seen in the window between two extracts anyway. One real probe per game tick is kept and the rest reuse the deducted result, which is exactly the safety semantics of "pull once per game tick". That chain cost 612 ms (1.02%) in the report.
  - **Candidate decision caches now key on item identity.** Smelting-recipe and tag-filter decisions were both cached by full `AEItemKey`, whose `equals` runs `ItemStack.isSameItemSameComponents` (wrapped by geckolib's mixin when that mod is present); the method's self time was 1416 ms (2.36%) in the report, the fifth hottest on the server. Smelting decisions now key on the item reference when the stack has no component patch and fall back to the full key when it does, keeping both correctness and hit rate; tag decisions take only the item as input to begin with and key on the item reference outright. This matches what the comb-processability cache already did — these were the last two places still keyed by `AEItemKey`.
  - **The Mekanism smelting lookup handle is no longer re-resolved on every miss.** Reaching the lookup entry point went through a registry resolve (`DeferredHolder.value()`) on every cache miss, costing 320 ms (0.53%) of self time in the report — about as much as the actual recipe queries. The handle is constant within one recipe-reload cycle, so it is now cached alongside the recipe version and replaced together with it (holding the old handle would answer from a stale recipe table).
  - **Classification travels with the candidate instead of being recomputed for sorting.** Sorting needs to know whether a candidate is a comb or a smelting input, and that was obtained by re-running classification — an extra pass through the recipe and tag caches on every pull. Classification depends only on candidate identity, not on sort order, so it is now written into the entry during admission and carried along.
  - **Slot admission is decided once per round.** "Will this slot accept this item" used to be called per candidate type *per slot*, even though a Mekanism factory's input slots are all created in one loop from the same machine's same method and therefore all accept or all reject. It is now decided once per item per round, guarded by the slot implementation class so a third-party custom slot triggers a fresh decision. That duplication cost 236 ms in the report.
  - **Candidate de-duplication uses a hash set.** The candidate list reaches thousands of entries on a large network while the selection cap is "process count × 2", so linear de-duplication made a full scan pay O(candidates × cap) item-key comparisons — the other source of the `AEItemKey.equals` time above.
- **The three per-candidate boolean caches share one lock-free generational table**: smelting-recipe, tag-filter and comb-processability decisions were all "synchronized plus an access-ordered LinkedHashMap". On a path taken thousands of times per tick that combination carries three fixed costs: entering and leaving a monitor on every read and write (even though only the tick thread truly contends), relinking list nodes even on a *hit*, and a boxed Boolean value. The new container demotes a full generation to "previous" once it fills and recycles the old array; lookups go current → previous and a previous-generation hit is promoted back. Eviction is therefore an O(1) whole-generation drop, hot keys survive any number of demotions, there is no hit-rate collapse from "clear everything when full", and residency is still bounded at twice the per-generation cap. A grid callback asking for invalidation only bumps a counter, with the actual clear deferred to the tick thread's next lookup — so no lock remains on the hot path at all. Smelting decisions alone accounted for 344 ms (0.57%) of self time in the report, most of it those fixed costs rather than real recipe queries.

#### Tests

- **Machine and upgrade regressions in this batch**: cover batch energy accounting, capacity budgets, deferral arithmetic, essence compression rules, raw-ore conversion arithmetic, feeder disabling and apiary slot geometry. Additional source-wiring assertions cover direct output, overflow, accelerated smelting, apiary transfer caches and AE2 comb entries; these check call structure and do not replace in-game integration tests.
- **Migration regressions**: all 68 legacy tier keys migrated, invalid values falling back to defaults without discarding valid siblings, pre-balance saves, partial migration when some split files were already edited, optional integration keys always present, transactional commit and rollback, interrupted-migration recovery, refusal to overwrite an existing backup, and world scope taking priority over the root config.
- **Backup rotation and overlay migration**: backup slots rotate (`.migrated.bak` → `.migrated-2.bak` → …), a modpack shipping the same file again is detected by content, player-edited keys survive an overlay of new pack values, and an unparsable previous backup degrades to filling in default-valued keys only.
...
- **Bounded LRU eviction semantics**: real unit tests (not source assertions) for `BoundedLruMap` cover the cap never being exceeded, eviction picking the least recently *used* rather than the earliest inserted entry, a hot key surviving continuous cold-key churn, rewriting an existing key refreshing its recency, a non-positive cap throwing, four concurrent writers still respecting the cap, and `putIfAbsent` converging on one instance. This is the correctness basis for the "clear-when-full → LRU" change.
- **Recipe lookup and component resolution wiring**: assertions pin down that the dye lookup goes through the index and no longer scans all recipes, that the index publishes an immutable snapshot and keeps the "first match wins / skip empty results" semantics, that a recipe reload invalidates it, that the input validation cache covers the full process count, and that the product-ownership cache is bounded with the expensive resolve outside the lock. On the component side they pin down that none of the ten hot paths falls back to `DeferredHolder.get()`, that `matchesComponents` resolves the component type once per call, that the accessor is lazy with no invalidation entry point (the static registry is constant), and that registration-time paths keep the original call form. Reverting any of these leaves behaviour fully correct while the cost quietly returns, which pure logic tests cannot catch.
- **Lock-free generational memo semantics**: real unit tests for `BoundedBooleanMemo` cover the three-state contract (unrecorded must be distinguishable from recorded-false), a previous-generation hit being promoted back so only the coldest generation is dropped, residency never exceeding twice the per-generation cap, `requestClear` taking effect on the next access without affecting later writes, an invalidation request posted from another thread being observed by the tick thread (the AE2 grid-callback case), `clearNow` dropping both generations, null keys treated as unrecorded without polluting the table, and a non-positive cap throwing. This is the correctness basis for the "synchronized LRU → lock-free generational table" change.
- **AE2 pull hot-spot wiring (this round)**: assertions pin down that all three boolean decision caches use the generational table with no `synchronized` on the hot path, that smelting decisions branch on "has a component patch" and cache the Mekanism lookup handle alongside the recipe version, that tag decisions key on the item reference, that the reserve probe goes through the per-tick memoised entry point and that a committed extract deducts from both tables, the cap-truncation rule (reuse only when "the cached cap is not smaller" or "the result was not truncated"), that classification travels with the entry and is never recomputed for sorting, that slot admission is memoised per round guarded by the slot implementation class, that cursor scanning de-duplicates with a hash set reused across ticks, and that only an O(1) free-slot check runs before an extract while the post-extract fingerprint uses the non-throwing encoder. Reverting any of these leaves behaviour correct while MSPT quietly climbs back.
...
- **熔炼输入标签表达式过滤**：AE2 拉取配置新增独立的熔炼输入标签过滤窗口，支持白名单/黑名单、`&|^!()` 运算符和 `*` 通配符；可从物品标签选取器快速添加或移除标签，也可直接填写物品 id。表达式经客户端与服务端双重长度、深度和节点数校验，蜜脾过滤语义不受影响。
...
- **AE2 输出推送职责拆分**：将原有大型推送器拆分为逐槽提交、同键合并、直推会话、账本结算、退避日志和复用缓冲等协作组件，保持“预留—提交—确认”语义并降低单类维护复杂度。
- **多类型输入公平调度**：AE2 输入拉取增加按空槽车道与配额的两轮分配，避免某一物品类型长期占满高容量输入槽而饿死其他候选；游标扫描和精确过滤键索引也同步接入。
- **配置与状态同步**：标签过滤表达式纳入方块实体存档、机器拆装、等级升级、配置卡和网络同步；服务端配置监听限定为本模组 SERVER 配置，客户端同步事件不再误触发本地文件迁移逻辑。
...
- **蜂箱批量刷新尖峰**：按本刻批量倍率调整累积刷新阈值，避免 CREATIVE/高倍时间加速下每刻反复触发固定刷新开销；喂食槽转化开关现随配置卡复制保留。
- **零耗时配方重复计算**：CREATIVE 升级下的零耗时 Mekanism 配方采用按方块的合并窗口，按获批操作数折算虚拟刻进度与能量，避免高倍加速重复执行完整配方计算。
- **低 TPS 闸门误停机**：将 AE2 输入在低 TPS 时的二值停机改为限流放行，并钳制非法 tick 样本，防止一次异常时间戳使降级状态永久保持。
- **异常日志与缓存生命周期**：高频蜂箱/AE2 异常改为时间节流；槽位上限、指纹、熔炼配方和网络成本缓存均设置版本失效或有界清理，避免跨存档残留和无界增长。
...
- **Smelting-input tag expressions**: the AE2 pull configuration now has a dedicated tag-filter window for smelting inputs, with whitelist/blacklist expressions, `&|^!()` operators and `*` wildcards. An item-tag picker can add or remove tags, while item ids may still be entered directly. Client and server enforce bounded length, depth and node counts, and comb matching remains unchanged.
...
- **Split AE2 output responsibilities**: the former monolithic pusher is now organized into slot and merged submission paths, direct-push sessions, ledger settlement, backoff diagnostics and reusable buffers while retaining reserve-commit-confirm semantics.
- **Fair multi-type input scheduling**: AE2 input pulls use two-pass empty-lane and quota allocation so one high-capacity item type cannot occupy every input slot; cursor scanning and exact-key indexing are wired into the same path.
- **Configuration and state synchronization**: tag expressions persist through block-entity saves, machine pickup, tier upgrades and configuration cards, and are synchronized over the network. Server config listeners now accept only this mod's SERVER config, so client sync events cannot trigger local-file migration logic.
...
- **Apiary batch spikes**: accumulation thresholds now scale with the current batch multiplier, preventing CREATIVE/high-speed operation from flushing the same fixed overhead every tick; the feeder conversion toggle is also copied by configuration cards.
- **Repeated zero-duration recipe work**: CREATIVE zero-duration Mekanism recipes use a per-tile coalescing window that derives virtual-tick progress and energy charges from approved operations, avoiding repeated full recipe calculations under acceleration.
- **Low-TPS false shutdowns**: the AE2 input gate now rate-limits instead of stopping completely at low TPS, and invalid tick samples are clamped so one bad timestamp cannot permanently force degraded mode.
- **Exception and cache lifecycle**: high-frequency apiary/AE2 errors are time-throttled; slot limits, fingerprints, smelting lookups and network-cost state have bounded storage and explicit invalidation to prevent cross-save residue or unbounded growth.
...
- **AE2 输入库存模式**：每个直连条目现在可以独立保留 AE2 网络库存；库存模式与无限拉取解耦，仍可分别设置每次拉取数量和无限拉取。保留量支持完整 `long` 范围并随过滤配置、配置卡和机器拆装持久化。
- **AE2 输入配置 GUI**：齿轮保持 AE2 齿轮图标，库存模式使用独立文字按钮；单项支持右键切换库存、Shift+右键设置保留量，批量齿轮支持一键设置数量/保留量/模式。tooltip 最后一行压缩为单行 `拉取/库存/保留`，并以 `∞` 显示无限值。
- **AE2 网络库存可见性**：配置界面显示标记物品当前可拉取库存；服务端使用 holder 级库存快照、每游戏刻模拟提取缓存和网格变更失效，避免每台机器持续遍历整个网络。
- **AE2 拉取批处理**：同一 AEItemKey 每轮仅执行一次网络提取，再在本地分发到多个输入槽；候选键降频刷新，并复用物品探针、扫描缓冲和慢网络退避，降低常态吞吐与高倍加速下的对象创建和 AE2 API 调用。
- **AE2 输入候选键扫描降频**：大型网络中每台蜂箱/离心机每 10 游戏刻刷新一次蜜脾类型候选列表；实际拉取仍按当前 KeyCounter 数量查询。高倍加速和慢 AE2 网络不再反复枚举整张库存，新加入的蜜脾类型最多延迟半秒被发现，网格切换会立即失效缓存。
- **AE2 能量缓存可真正充满**：移除四批需求的注入目标限制，机器会在容量按当前 ENERGY 升级归一化后补足完整差额；空闲机器也能从 AE2 创造能源原件充满，不再因当前工作需求为零而跳过充能。
- **AE2 取电遍历减半**：Applied Flux 与 AE2 原生能量路径均从 `SIMULATE + MODULATE` 两次访问改为单次有界 `MODULATE`；请求始终不超过正常容器剩余容量，实际取得多少就安全注入多少。
- **移除重复需求扫描**：补能不再为蜂箱或离心机重复计算当前批次能耗，处理前的满容量检查会直接短路，处理后仅补回实际消耗的差额。
- **MCP 性能复核与 AE2 热路径**：对 `QgFuv`、`Zi1UM8tszQ`、`HpFapq178W`、`IDOnT` 四份 Spark/Observable 报告重新分析；AE2 拉取仍低于 1% 级别，工厂/PB 处理和外部时间扳手实体才是主要成本。所有 AE2 宿主现在直接返回稳定状态持有者，能量容量归一化增加按机器缓存，减少 JDTE 子 tick 下的接口分发和重复升级计算。
- **输出槽标志增量聚合**：批量输出期间记录 listener 事件与显式槽位快照，常规 PB flush 不再因同步 listener 把单进程更新退化成整台工厂扫描；无法定位的计划或外部变更仍保留保守刷新。

### 变更

- **无限拉取按条目生效**：单个条目的无限设置不再扩大整台机器的配额；Shift 设置的全局无限模式继续可用。
- **能量容量缓存**：缓存加入 Mekanism 升级状态指纹，升级安装/拆除后会立即重新计算容量。

### 修复

- **AE2 保留量一致性**：同一游戏刻内实际提取会立即扣减对应缓存键，避免 JDTE 高倍率重复调用继续使用旧模拟库存并突破保留量。
- **输出槽状态**：批处理期间遇到无法定位来源的监听器事件时改为保守全量刷新，避免其它进程的满槽/物品计数陈旧。

### 性能

- **批量 AE2 交互**：同一 `AEItemKey` 每轮只执行一次 ME 提取并在本地分发；慢网络按预算和键级退避，减少常态大量吞吐及 JDTE 256 倍加速下的重复 API 调用。
- **MCP 复核**：最新 Spark/Observable 报告显示 AE2 输入路径仍为低占用路径，主要负载来自高倍率 Productive Bees/Mekanism 工厂和主机内存交换；本次优化针对已识别的 AE2 热点进行局部降频与缓存修正。
...
- **AE2 input stock mode**: Each direct entry can keep an independent AE network floor. Stock mode is decoupled from unlimited pull, so per-pull amount and unlimited behavior remain configurable. Full `long` reserve values persist through filter data, cards, and machine removal/reinstallation.
- **AE2 input configuration GUI**: Gear controls keep the AE2 cog icon while stock mode has a dedicated text button. Per-entry right click toggles stock mode and Shift+right click edits the reserve; the batch controls support one-click amount, reserve, and mode changes. The final tooltip row is compact (`Pull/Stock/Keep`) and unlimited values render as `∞`.
- **AE2 stock visibility**: The configuration screen shows currently extractable stock for marked items. Holder-level snapshots, per-tick simulated-extract caching, and grid-change invalidation avoid a persistent full-network scan per machine.
- **AE2 pull batching**: Each AEItemKey performs one network extraction per round and distributes locally across input slots. Candidate keys, item probes, scan buffers, and slow-network backoff are reused or bounded to reduce AE2 API calls and allocation pressure during sustained throughput and accelerated ticks.
- **Lower-frequency AE2 input candidate scans**: Each apiary/centrifuge refreshes its comb-key candidate list every 10 game ticks while pull amounts still come from the current KeyCounter. Accelerated machines and slow AE2 networks avoid repeatedly enumerating the whole inventory; newly-added comb types may take up to half a second to appear, and grid changes invalidate the cache immediately.
- **AE2 can now fill machine buffers completely**: the four-batch target is removed. After capacity is normalized from currently installed ENERGY upgrades, AE2 refills the full remaining difference; idle machines can also charge fully from Creative Energy Cells.
- **Half as many AE2 power traversals**: Applied Flux and native AE energy now use one bounded `MODULATE` call instead of `SIMULATE + MODULATE`. Requests never exceed normalized remaining capacity, and only the amount actually extracted is injected.
- **No repeated demand scan for charging**: energy refills no longer recalculate current recipe demand. A full pre-processing buffer exits immediately, while the post-processing check replaces only the energy actually consumed.
- **Same-tick stock accuracy**: committed extractions update the cached key immediately, so repeated JDTE calls cannot cross a reserve floor using a stale simulation.
- **Per-entry unlimited pulls**: enabling unlimited on one entry no longer gives unrelated combs the machine-wide unlimited quota; the legacy Shift global mode remains available.
- **Upgrade-aware energy cache**: the normalized capacity cache now includes the Mekanism upgrade state, so installing or removing upgrades recalculates the capacity immediately.

#### Changed

- **AE2 hot path**: candidate keys, inventory probes, and per-key extraction batches are reused or bounded; AE2 hosts return their stable state holder directly to reduce accelerated-tick dispatch overhead.
- **Conservative output refresh**: unidentified listener changes during an output batch trigger a full refresh, preserving correct state across all factory processes.

#### Performance

- **Batched ME interaction**: each `AEItemKey` performs one ME extraction per round and distributes locally. Slow networks use shared budgets and key-level backoff, reducing repeated API calls during sustained throughput and 256x acceleration.
- **MCP review**: the latest Spark/Observable reports still place AE2 input below the main cost; high-tier Productive Bees/Mekanism factories and host memory pressure dominate. The release targets the measured AE2 hotspots with bounded scans and cache corrections.
...
- **无 AE2 环境的网络隔离**：仅当 AE2 已加载时注册 AE2 输入、输出、筛选与同步数据包，避免可选依赖缺失时创建指向 AE2 API 的处理器方法引用；离心机熔炼兼容开关改由不链接 AE2 API 的独立处理器承载，未安装 AE2 时仍可正常使用。
- **启动稳定性**：蜂箱、离心机和界面相关的运行状态改为首次使用时准备，兼容更多整合环境的加载顺序，避免进入游戏时发生初始化异常。

### 性能

- **离心机流体回推吞吐**：关闭“产出直接返回 AE2”时，流体输出改为每个真实游戏刻立即推送，不再等待 20 tick 批处理窗口，避免本地流体罐成为高倍率离心机的吞吐瓶颈；直接产出模式继续保留合并批处理以控制 AE2 存储调用开销。
- **高并行本地流体排空**：高等级机器关闭“产出直接返回 AE2”时，会在配方处理前排空遗留流体，并在本地罐无法容纳下一候选批次时于同一真实游戏刻继续回推 AE2；额外排空受每 tick 64 次预算和既有网络拒收退避保护，256 倍加速仍合并为真实 tick 调度，不再由单次本地罐容量限制 16 STACK 升级的批次。

> **首个正式版**：模组内部版本、构建版本和 CurseForge 正式版本统一为 `1.0.0`，上传同一个
> `productivebeesgenesis-1.0.0.jar`。

本版本汇总 `2.0.9` 后的所有开发批次，并参考本地 AE2 Lightning Tech、Data Energistics、JDTE、
NeoECOAEExtension、UselessMod、Mek-Energistics、Mekanism 与 Productive Bees 源码，对机器处理、蜂箱生产、
AE2 交互、持久化、兼容性和发布流程进行了系统优化。

### 全面优化汇总

- **JDTE 合并虚拟 tick** — 蜂箱、基础离心机和全部工厂等级共享虚拟 tick 银行、真实游戏刻门控与批量预算；
  未加速时走单 tick 快路径，加速停止后的挂账限制为 4096，避免残留长时间继续加速。
- **轻量熔炼批量推进** — SMELTING 通道复用已缓存配方并通过 `CachedRecipeBatchAccelMixin` 合并推进，
  跳过重复 ejector、能量槽回填、配方重查和输出检查；注入失败可安全回退原版逐 tick 行为。
- **批次语义统一** — 万象创世蜜脾与普通 Productive Bees 配方统一使用虚拟 tick 计划、跨周期进度、
  输出受阻裁剪与能量/输入限制，避免加速倍率重复放大或丢弃剩余虚拟 tick。
- **机器能量预算与公平性** — 蜂箱、离心机和工厂的批次需求只计算一次，本地容量扩展与 AE2 差额提取共享结果；
  多通道按剩余能量加权分配并轮转起点，避免前部槽位长期独占能量。
- **蜂箱生产热路径** — 多花材、琥珀实体和随机产物使用缓存快照与批量采样；高倍率生产通过聚合、饱和运算和
  稳定轮转减少逐次随机、线性扫描与临时集合分配。
- **蜜脾块原地转换** — 普通/特殊蜜脾块查找使用索引，转换后的同组件栈就地缩减或替换，减少高产量批次中的
  `ItemStack` 复制、拆组和中间列表。
- **蜂箱输出缓冲复用** — 部分转移、外部弹出和 AE2 部分接收后原地缩减缓冲栈；新增单栈 `offer` 快路径，
  只在实际合并或入队时更新版本与脏状态，降低失败重试的分配和保存压力。
- **蜂箱直连离心机守恒** — 提取、接收、拒收和缓冲净额统一记账，异常时回填源槽；连续失败自动交还
  Mekanism Ejector/AE2，部分失败保留重试状态，确保物品不丢失、不复制。
- **稳定槽位与分页输出** — 工厂输出使用稳定的有序槽位索引、分页容器槽和轮转遍历，避免页切换、动态槽数和
  高并行输出导致重复全表扫描或前部槽位饥饿。
- **AE2 输入有界扫描** — 库存快照使用游标前缀有界扫描，直连白名单按 key 查询；过滤器预解析并在单遍中完成
  白/黑名单、精确/模糊匹配和单次拉取上限计算，候选结果与临时缓冲跨 tick 复用。
- **AE2 输入槽容量快照** — 每个候选类型只计算一次各输入槽可接收容量，规划和实际插入共享 `long[]` 快照；
  整批进入单槽时直接传递 remainder，避免重复 `getStack/matches/getLimit` 与 `copyWithCount`。
- **AE2 输入候选 O(1) 统计** — 候选数量使用复用的 `Object2IntOpenHashMap`，公平调度服务量使用
  `Object2LongOpenHashMap`；每 tick 可提取库存视图同样使用 primitive long 缓存；多槽同 key 合并为一次
  ME extract 后再本地分发，消除线性反查和 `Long` 装箱。
- **AE2 输出分组与退避** — 物品输出使用保留物理槽顺序的 `Object2LongLinkedOpenHashMap` 聚合，按 key 记录
  成功/失败退避；单个不可接收类型不再拖慢整台机器，部分成功严格按实际接收量缩减本地库存。
- **AE2 流体批处理** — 待推送流体使用双 `Object2LongOpenHashMap` 交换排空，primitive entry 遍历消除热路径
  `Long` 装箱和 flush 复制；请求按每 key 有界分批、按实际库存 clamp 并在接收后精确 shrink。
- **AE2LT 过载接口原生存储通道** — 全部离心机与工厂向相邻设备提供按面的原生 `MEStorage`，
  过载接口优先命中该通道，不再构造并周期刷新通用物品/流体 capability wrapper；物品输出按内容版本共享快照，
  流体键按槽复用，输入、物品提取和流体提取分别轮转起点，同时动态遵守 Mekanism 侧面配置。
- **AE2 大数安全** — 物品、流体和能量的累计、请求、注入、提取与 shrink 统一使用饱和运算和请求上限，
  防止高倍率下 long/int 溢出、第三方存储异常返回超量及大请求独占单 tick。
- **持久化与升级迁移** — 蜂笼、能量槽、输出缓冲、AE2 过滤数量/无限标志、多流体槽和选择槽位完整跨等级迁移；
  损坏或旧格式 NBT 使用有界解析与兼容回退，重复掉落序列化具备幂等保护。
- **可选依赖与启动安全** — JEI、JDTE、Mekanism Extras、Evolved Mekanism、Applied Flux、Iris 等调用点增加
  加载/链接守卫；高风险 Mixin 注入使用可选匹配与安全降级，避免未安装或版本差异导致启动崩溃。
- **交互与网络安全** — AE2 配置界面数量、无限拉取、全局齿轮和虚拟输出槽统一服务端距离、容器一致性、
  频次与资源 ID 校验；缓存失效、Shift 提取容量和超大数量显示得到修复。
- **新增正式版内容** — 完成万象创世相关物品/方块/生成蛋资源、无用副产物销毁升级、蜂箱离心优先级控制、
  平衡预设与兼容迁移，以及相应中英文文本、模型、配方和进度解锁。
- **模组列表描述本地化** — 简化模组简介；中文游戏显示中文描述，其他语言回退到简洁英文描述。
- **发布包整理** — 发布 JAR 排除本地材质备份、预览文件和调试符号，仅保留运行所需资源；构建归档继续保持可复现。
- **发布产物自检** — 构建自动写入版本 `1.0.0` 和 CurseForge 版本 `1.0.0`，JAR 内打包 MIT 与第三方许可，
  `verifyReleaseArtifact` 校验 Manifest、NeoForge 元数据、图标、许可证和双版本映射；归档固定条目顺序并移除
  文件时间戳，保证同一源码的发布 JAR 可复现校验。

### 离心机加工反馈与性能批次

进度条、JDTE 加速残留、万象创世蜜脾虚拟加速进度和 SMELTING 配方时间加速的详细修复如下。
...
- **离心机紧邻 AE2LT 过载接口的物品/流体直通优化（MAJOR）** — 参考 AE2LT
  `AE2NativeMachineAdapter` 对 `AECapabilities.ME_STORAGE` 的优先解析顺序，为基础离心机、原版工厂及
  EM/ME/EME 全部工厂注册原生 `MEStorage`。过载接口现在直接读取离心机输出槽和流体槽，绕过
  `StackWorldBehaviors` 通用 facade 的 capability 扫描、20 tick wrapper 重建及重复模拟提取；
  物品可用量按 `outputContentsVersion` 缓存并在真实变化后才重扫，流体复用按槽
  `AEFluidKey`，所有方向共享快照和轮转游标。接口向机器供料时直接分发到有效输入槽并使用
  `AutomationType.EXTERNAL`，输出物品、输出流体和输入物品分别公平轮转；每次查询和传输均重新检查
  Mekanism 的朝向、启用侧和 `INPUT/OUTPUT/INPUT_OUTPUT` 数据类型，旋转或修改侧面配置无需重建适配器。
  大请求使用饱和 long/int 转换，提取按真实槽位变化量记账，流体提取完成后回收空多流体槽，保持
  模拟/执行一致且不丢失、不复制。
- **JDTE 批量预算快路径与首次刷新修复** — 未加速机器现在直接消费当前基础 ticker 入账的单个虚拟 tick，
  不再每 tick 检查配置、TPS 采样与饱和告警；存在加速挂账时才进入自适应预算慢路径。同时修复
  `Long.MIN_VALUE` 初值参与 tick 差值计算时溢出，导致首次 TPS 自适应预算与饱和告警不刷新的问题，
  并在 tracker 重置时同步清空预算缓存。
- **加速批次能耗需求只计算一次** — 蜂箱、基础离心机与工厂统一在批次能量入口计算一次需求，
  同一结果同时用于本地容量扩展和 AE2 差额提取，避免 JDTE 256x 批次下重复计算升级、进程数与并行倍率；
  未安装或关闭 AE2 时仍会扩展本地容量，保持批量加工语义。
...
**JDTE 兼容批次（2026-08-13，已并入 1.0.0）**
...
**稳定性与全量审查批次（2026-08-12 至 2026-08-14，已并入 1.0.0）**

该批次首先修复 issue #6 报告的崩溃：给非蜂箱/离心工厂机器安装 ME 工厂升级时，
...
### 本轮性能与交互优化（2026-08-14）

- **AE2 输入 100-tick 公平调度** — 新增 `Ae2InputFairnessScheduler`，按当前 100-tick 窗口内已服务量对可拉取类型排序，优先拉取服务量最少的蜜脾类型；在保留 marked/蜜脾块优先排序的基础上，替代原先仅按 typeRotation 起始轮转的方式。
- **工厂多槽输入单次 ME extract** — `Ae2InputPuller.pullBatchForType` 对同一 `AEItemKey` 先做一次 ME 网络提取，再本地分发到多个输入槽，高等级工厂的 AE2 API 调用次数显著下降。
- **按 `AEItemKey` 失败退避** — 新增 `Ae2InputKeyBackoffRegistry`，单个蜜脾类型 ME 提取失败只暂停该 key，不再由 per-tile `returnBackoff` 拖慢全部输入拉取。
- **外部物流 `getLimit` 头部缓存** — `BasicInventorySlotMixin` 改为 HEAD 缓存直出，跳过原版 `ItemStack.getMaxStackSize()` 组件链；`SlotLimitCache` 高频路径去锁。
- **无标记全量无限拉取** — 全局齿轮 Shift 点击在无任何过滤标记时可切换 `unlimitedAllFallback`，实现拉取所有蜜脾无限量；该状态同步到客户端并持久化，信息面板显示全量无限状态。
- **客户端全局齿轮视觉反馈** — `GlobalGearButton` 在全量无限激活时显示高亮齿轮与 `∞` 标记，并更新中英文 tooltip。

### 发布版本映射

- 以上开发批次统一发布为正式版本 `1.0.0`，CurseForge 渠道为 `Release`。
...
- **AE2 拉取 GUI 数量数字显示（MEDIUM）** — 第三行虚拟槽数量基于 AE2LT 过载接口 `LargeStackCountRenderer` 调整：固定 0.7 缩放、右对齐到槽位右缘、K/M/B 紧凑缩写（单位值 < 10 时保留 1 位小数，如 1.5K），数字不超出格子且各数量视觉一致；同步修复 `NumberFormatter` 999999 显示为 "1000.0K" 的四舍五入进位缺陷并新增单元测试
...
- **多流体推送效率与扣减（CRITICAL）** — 流体槽采样、满载扫描和多流体类型预留改为每个真实游戏刻执行一次，加速子 tick 不再重复计算；推送改为单次 `long` 数量插入，按 AE 实际接收量从本地槽精确扣减，失败流体留在本地下刻重试
- **外部接口集中灌入（MAJOR）** — 新增工厂外部插入公平策略：每台机器、每种物品每个游戏刻只开放一个稳定目标槽和约一刻需求额度，缓解 AE2LT 过载接口因固定连接顺序把蜜脾/蜜脾块集中发给少数机器的问题；手动输入与机器内部 AE 拉取不受限流
- **多输入槽并行公平（MAJOR）** — PB 配方处理起点每刻轮转，并补齐 ME / EME 工厂遗漏的产量并行倍率，避免高等级工厂右侧输入槽长期不处理或处理缓慢
- **绝对蜂箱能量不足（MAJOR）** — 蜂箱能量容量按实际蜜蜂槽位扩展，256 倍加速下不再只有前 17 只蜜蜂工作；32 个 MEK 速度升级恢复到旧版 26 tick 的工作节奏
- **蜂箱缓存人为溢出（MEDIUM）** — 大产物栈不再在内存中按 99 拆成多组，同物品同组件先合并再入队，避免 8808 个产物被拆成 89 组后触发 64 组上限淘汰
- **电炉兼容缓存（MEDIUM）** — 切换总开关或单机开关后立即清除 Mekanism 已缓存配方并唤醒配方监视器，不再继续执行旧配方

### 新增

- **蜂箱/离心机产物直输 AE 开关** — 可让产物直接回到 AE 存储，也可以保留先写入输出槽缓存的模式，方便把蜜脾直连离心机再处理或送到其他机器
...
- 流体输出不再因低 TPS 完全停止，避免把输出背压反馈成流体槽打满停机
- 持续满载诊断改为每分钟最多一次，不再出现每 5 秒刷屏
- 热路径复用普通 Map 并移除并发容器开销，减少高并行工厂每刻分配
- 模组体积优化：图标转 256 色 PNG 调色板，其余纹理剔除冗余 ICC/DPI 元数据并无损重编码，资源体积从约 800KB 降至约 178KB

### SemVer 合规性

- 本次以修复与性能优化为主，延续 2.0.x 维护线定为 **PATCH** 级别（v2.0.7-hotfix → v2.0.8）
## [dev-2.0.7-hotfix] - 2026-08-06
...
## [dev-1.0.0-internal] - 2026-06-15

> 早期内部开发版本，仅用于测试，未作为 CurseForge 正式版发布。
