...
- **AE2 拉取 GUI 数量数字显示（MEDIUM）** — 第三行虚拟槽数量基于 AE2LT 过载接口 `LargeStackCountRenderer` 调整：固定 0.7 缩放、水平居中于槽位、K/M/B 紧凑缩写（单位值 < 10 时保留 1 位小数，如 1.5K），数字不超出格子且各数量视觉一致；同步修复 `NumberFormatter` 999999 显示为 "1000.0K" 的四舍五入进位缺陷并新增单元测试
