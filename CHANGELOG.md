...
- **服务端配置拆分为三个文件**：原来的 `productivebeesgenesis-server.toml` 拆成 `productivebeesgenesis-gameplay-server.toml`（蜜蜂属性、获取、转化、产出、平衡规则、高级蜂箱）、`productivebeesgenesis-machines-server.toml`（离心机与机械蜂箱的基础参数、弹出策略、IO 限流、AE2 集成、升级上限、熔炼兼容）和 `productivebeesgenesis-capacities-server.toml`（容量矩阵）。原因：单文件已超过 24,000 字节，配置界面一屏塞不下，玩法与机器参数混在一起难以分发；拆分后整合包作者可以只覆盖需要的那一份。
...
- **Stricter release artifact verification**: the build now checks that the JAR stays under 1,800,000 bytes, that the mod icon is at most 32 KiB and 256×256, and that no datagen classes or removed duplicate resources are present, and writes a size breakdown report. The current artifact is 1,747,152 bytes.
