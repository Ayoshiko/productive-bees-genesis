...
* 配方O(1)索引 — CentrifugeRecipeIndex 避免每tick全量遍历
* AE2/管道限流包装器 — RateLimitedItemHandler 防止高频拉取导致tick延迟
* 蜂箱产物路径优化 — BeeHelperMixin 短期缓存+对象池+合并堆叠+可选节流
* hasCentrifugeRecipe输入复用 — AbstractCombEventHandler 避免重复配方查找
* 性能监控命令 /pbg perf — PerfCommand 支持Spark profiler集成
* 性能监控JMX MBean — PerformanceMonitor 暴露tick时间/缓存命中率/能量消耗
* 离心机标志位优化 — 输出槽hasOutputItems/outputSlotsFull避免每次遍历
* 配方缓存优化 — RecipeCacheManager 使用Record CacheKey+LRU淘汰（LinkedHashMap>1024）
* 客户端事件处理器模板方法模式 — AbstractClientCombEventHandler基类
* 宇宙渲染队列预分配矩阵 — CosmicRenderQueue复用REUSABLE_OLD_PROJECTION/MODEL_VIEW
* 离心机能量计算基于实际能量差 — 摆脱getLastUsage父类语义依赖
* PbRecipeProcessor缓存energyPerTick/operationsPerTick — 避免循环内重复Math.pow
...
* JMX MBean注册异常 — 改用StandardMBean显式绑定接口，实现类改为static内部类
* 流体槽满日志刷屏 — 256倍加速下INFO日志每tick数十条，改为静默丢弃
* 配置文件不正确警告 — 移除dead config旧键残留，删除旧toml文件
* 配方序列化NPE兜底 — 五种PB配方toNetwork的Mixin HEAD拦截+fallback
* BeeRecipeReloader未就绪时跳过替换 — 避免toNetwork崩溃
* getBeeIngredient containsKey校验 — 类型不存在时回退到minecraft:bee

### refactor
* 综合代码审计 — 合规性、稳定性、性能、代码标准 ([05c9112](https://github.com/Ayoshiko/productive-bees-genesis/commit/05c9112))
* 工厂标志位管理逻辑抽取 — 3个工厂类约90行重复代码提取到MekCentrifugeFactoryHelper
* GUI Helper组合模式 — GuiMekCentrifugeFactoryHelper工具类抽取widget创建逻辑
* PbRecipeContext接口扩展 — 6个新方法供工厂版PbRecipeProcessor统一调用
