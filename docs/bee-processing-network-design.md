...
**后续设计约束：** 当前是固定布局 1，不开放数据驱动模板重载。审计仍采用保守撤销后重新验证；M04 接生产前应评估只读审计通过时保持有效资格，避免完整机器周期性停产，同时保留变更即失效与检查失败立即暂停。M06 可先推进静态状态展示及真实客户端验收；M04 继续受 D16c／D17 前置约束。没有新的 Spark／MSPT 结论，也没有验证真实玩家多端交互。
