...
候选实现 A 为 EAEP 1.21.1／ECO 当前采用的稀疏大数分层：绝大部分键走原生 long，少量超范围键进入 BigInteger 区。候选 B 借鉴 AE2LT／Thunderbolt：不可变键映射到 entryId，数量、脏标记、序列化键缓存放分段数组，再配稀疏大数区。B 避免对同一键反复哈希和大表扩容复制，但复杂度更高；P0 基准决定是否采用，不凭源码名称宣称最快。
...
不设置 `maxTypes`／`maxStorageBytes`／产物数量容量配置，不将分段大小或 UI 页大小当作库存上限。entryId 使用可扩展分段寻址，回收空槽时附带 generation，避免旧预约指向复用后的另一物品。零余额且无预约／历史隔离引用的条目可回收。
...
本提供者可以把自己的加法安全饱和，却无法保证随后其他模组仍安全累加。必须测试 AE2 19.2.17、多个无限盘及不同挂载顺序；若原生网络聚合不安全，先交付可配置的桥接“显示投影”兼容模式或独立且可选的兼容修复。该模式不限制实际库存、实际可提取量或本模组终端精确显示，也不能伪称 AE2 原生已经支持任意精度。
...
**D04 — ProductKey 和任意精度数量。** 前置：D02。实现不可变组件键、数量升降级、分段索引、零键回收与 generation；普通计数路径不创建 BigInteger。以 BigInteger 参考模型跑随机增减序列，覆盖 0、Long.MAX_VALUE、126 位以上、哈希碰撞、同 ID 不同组件及槽复用。通过条件为所有正负边界有明确行为，不存在截断，存储实现不含类型／总字节容量条件。
...
**D09 — 持久化状态域。** 前置：D03–D08。新增 `NetworkDirectory`、`NetworkSavedData`、记录 codecs，保存所有账户、蜜蜂、喂食槽、升级、能力来源、在制计划和规则版本；索引从权威记录重建。测试 round-trip、未知版本、截断数据、缺键注册、域丢失和重复身份。通过条件为无法解码时隔离并保留原始数据，正常保存加载摘要一致，不默默生成空网络。
...
当前工作区用于网络分支；维护主分支可再建立同级独立 worktree，例如 `productive-bees-addon-maintenance`，两者各自拥有 `run/` 和测试世界。未提交修改不属于某个分支的持久历史，切换前应先完成明确的提交或保管，不能误带进维护工作。开发构建标为预览版本，默认通过服务端功能开关限制新网络形成。
