...
独立库存有限，不把网络无限账本复制进多方块控制器；安全脱网只交还这一机器的蜜蜂、食物、升级及可装入的有限物料，网络共享产物留在网络。空间不足或仍有未知交接时保持暂停。最初机器类型、具体长宽高范围、主核等级、单元数量、配方和能耗在 M02／M04 进入时明确，不能因预留框架自动承诺所有组合已支持。
...
| M02 具体模板与预算验证 | M01；先确定第一种机器类型／矩形尺寸范围／角色及唯一核心，再做纯状态查询扫描 | 每步读取次数受限；所有区块覆盖、空腔／朝向／重复核心／歧义候选、扫描中变更与旧定义拒绝；失败原因可定位 |
| M03 真实形成与生命周期 | M02；控制器／角色方块注册、机器目录、排他占位、事件索引、成形提交、部件绑定 | 专服形成／拆除／同边相邻／重叠、内部区块卸载、两轮重建、控制器重放、正常重启与旧 capability 失效；无重复 ticker |
| M04 独立工作及升级 | M03、D16c、D17；具体机器有限库存／FE／配方，真实升级与单机能力快照 | 无蜂业核心也能运行；产出／能耗与独立参考一致，满载退避、已付费保管、无掉落物、单机升级不影响另一台 |
