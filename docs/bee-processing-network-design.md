...
独立矩形多方块目前完成设计准备，M01–M07 尚待逐步实施；新增角色、机器身份、联网交接和核心动画合同不代表已有可玩的多方块机器。M 系列状态独立于 P3 验收，见 11.12。
...
| 步骤 | 前置与单步交付 | 独立验收 |
| --- | --- | --- |
| M01 几何与视觉锚点 | 本轮定义的两层结构合同；矩形尺寸、位置分类、控制器锚定四向变换、连续核心位置／有限渲染范围 | 非立方体、薄层、负世界坐标、四向逆变换、long 体积、溢出拒绝、可变坐标冻结；不注册方块、不开放生产 |
