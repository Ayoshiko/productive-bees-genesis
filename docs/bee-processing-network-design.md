...
状态：P2（D09–D12）已完成，见 10.21；P3／D13 已以 `583ecee` 提交推送，见 10.22–10.23；D14 基础蜂箱逐位喂食、共享组与迁移恢复已实现，见 10.24。D15a 共享内核已提交 `2495301`，D15b1 作业事务与严格恢复见 10.26；D15b2a 基础机封存升级与配方适配见 10.27，D15b2b 预算化异构分配和显式服务见 10.28；下一步 D15b2c 故障／重启及来源矩阵收尾。D15–D16 和 P3 阶段退出尚未完成，生产仅有显式开发入口，未接自动调度。日期：2026-09-20。D01 独立机测量基线：`f2ae0b8ba586d1a9f8ee6d512d959b4bc2fb1de6`，模组版本 `1.0.8`；开发分支为 `bees-processing-network/1.21.1`，维护分支为 `main-neo/1.21.1`。
...
**D15 拆分验收：** D15a 共享数量／周期／计费内核、物理回接和回归见 10.25；D15b 实现基础离心机的权威作业、精确投入／结果结算、异构 lane 分配、所有权迁移和严格 checkpoint 恢复。D15b 还须测升级变化／补料／缺电发生在周期中途、暂停与重启、物品＋流体、蜜脾块和自动离心来源防重入；实际外部 FE 口恢复供电按 10.28 的依赖修正放在 D16 端到端验收。D15a 单独通过不能解除 D16 的 D15 前置条件。

| D15b 子步骤 | 交付 | 退出条件 |
| --- | --- | --- |
| D15b1（已实现，见 10.26） | 固定周期作业、投入／结果原子结算、有限 FE 迁移、严格存档和交还守卫 | 各付费／冻结／结算边界恢复守恒，旧请求与坏数据被拒绝；领域验证不代替真实机器验证 |
| D15b2（进行中） | b2a／b2b 已完成（10.27–10.28）；b2c 故障与重启收尾 | 蜜脾／蜜脾块、物品＋流体、逐机升级、补料／断电恢复／中途升级／拆机／重启及自动离心来源防重入；真实专服与独立机对照通过，才完成 D15 |
