...
状态：P2（D09–D12）已完成，见 10.21；P3／D13 已以 `583ecee` 提交推送，见 10.22–10.23；D14 基础蜂箱逐位喂食、共享组与迁移恢复已实现，见 10.24。D15a 共享内核已提交 `2495301`，D15b1 作业事务与严格恢复见 10.26；D15b2a 基础机封存升级与配方适配见 10.27；下一步接异构分配与显式服务。D15–D16 和 P3 阶段退出尚未完成，生产仅有显式开发入口，未接自动调度。日期：2026-09-20。D01 独立机测量基线：`f2ae0b8ba586d1a9f8ee6d512d959b4bc2fb1de6`，模组版本 `1.0.8`；开发分支为 `bees-processing-network/1.21.1`，维护分支为 `main-neo/1.21.1`。
...
| D15b2（进行中） | b2a 基础机升级与 PB 配方适配已完成；b2b 异构 lane 分配和显式网络服务入口；b2c 故障与重启收尾 | 蜜脾／蜜脾块、物品＋流体、逐机升级、补料／断电恢复／中途升级／拆机／重启及自动离心来源防重入；真实专服与独立机对照通过，才完成 D15 |
