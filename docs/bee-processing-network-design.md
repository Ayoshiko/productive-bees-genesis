...
### 9.2 一体机内部生产演出（2026-09-25，待 M06b／c 实施）
...
**M06b 进一步拆分：** b1 先交付只含固定布局索引、方向、结构状态和身份／版本的原生 BE 展示帧，独立于权威 NBT；本次服务器会话共享单调发布号，不保存到世界，客户端按连接内版本拒绝旧帧并在区块卸载清理。结构帧仅在变化时借原版区块追踪更新，同位置同 tick 由原版合并，初始区块数据提供完整帧；编码不超过 256 字节，不含工作事件或资源列表，不增加独立发包 ticker。b2 再接内部静态场景、同源锚点和 BER；b3 交付纯时间轴及事件生命周期。9.2 的生产事件、按查看者汇总的活动流限频／字节预算和 M04 触发在首次开放工作演出前另行验收，不能将静态结构帧的原版合并当作活动流总预算。
...
| M06b3 纯时间轴与活动生命周期 | b2；有限事件描述、姿态采样、暂停／时间跳变及取消恢复；真实完成事件另需 M04 | 旧代际／重复／迟到事件、首次跟踪基线、卸载／重载／世界切换清理；开发演示不能伪造正式生产，开放活动包前验收共享网络预算 |
...
当前已完成 D16c2c 操作界面、M01–M03 固定布局一体机结构、M03d 稳定只读审计、M06a 静态状态、M06b1 展示帧与 M06b2 内部静态场景，见 10.45–10.54。网络主线下一步仍为 D16c3 真实玩家闭环，先明确最小授权事务，再验收实际登录、双玩家竞争和正常玩家文件重启。多方块下一步为 M06b3：实现纯时间轴、有限事件描述与暂停／时间跳变／取消恢复，随后按 M06c 分段绘制；9.2 已明确蜜蜂入箱到资源收纳的分镜、透明绘制、事件合并和客户端预算。M04 生产／插件继续等待 D16c、D17，真实生产演出不能由结构形成冒充；完整 M06、动画兼容与帧耗仍待后续联合验收。
