...
独立矩形多方块的 M01、M02、M03 已完成固定布局结构范围的联合验收，证据见 10.41、10.46–10.50。真实角色方块已能形成三尺寸一体机，区块与正常重启边界已验证；下一步可推进 M06 基础视觉，M04 生产／插件仍待 D16c 与 D17 前置。M 系列状态独立于 P3 验收，见 11.12。
...
**M06 分步合同（2026-09-25）：** M06a 先提供控制器五种粗状态：未形成、检查中、结构就绪、等待区块、异常。它们投影到原版方块状态，使用颜色加不同形状的静态标志；新观察者随区块同步取得状态，不为静态模型提前创建视觉包／实例缓存。结构就绪不等于正在生产；实际蜂箱／离心／直出活动等 M04 接入后再由已提交工作派生。M06a 同时验证观察窗渲染层、四向模型、资源重载和真实客户端状态往返。M06b 再接有代际的视觉快照、核心锚点与 BER；M06c 覆盖动画开关、连接材质、可选光影和多台开销的联合门。静态模型通过不自动完成整个 M06。
...
| M06 材质与核心动画 | M03、9.1；先同步粗状态和静态后备，再表面动画／BER／连接材质 | 真客户端验证四向锚点、遮挡／剔除、暂停、卸载、资源重载、可选光影、禁用效果与多台开销；专服无客户端类加载 |
...
当前已完成 D16c2c 操作界面及 M01–M03 固定布局一体机结构，见 10.45–10.50。网络主线下一步仍为 D16c3 真实玩家闭环，先明确最小授权事务，再验收实际登录、双玩家竞争和正常玩家文件重启。结构主线已完成真实形成、静默变更、审计公平性、区块与正常重启联合验收；下一步可推进 M06 基础状态／静态视觉和真客户端检查。M04 生产／插件继续等待 D16c、D17，并先解决保守审计对稳定生产的周期性暂停影响。主机核心材质与动画共用 M01 锚点，动画与性能验收仍按 M06 单独交付。
