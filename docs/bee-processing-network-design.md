...
独立矩形多方块的 M01 几何／视觉锚点及 M02a／b 一体机模板／预算验证已完成，证据见 10.41、10.46–10.47；继续 M03 真实形成与生命周期，M03–M07 尚待实施，目前没有可玩的多方块机器或动画渲染。M 系列状态独立于 P3 验收，见 11.12。
...
区块索引只记录已存在控制器的候选包围盒与形成结构；放置、破坏、转向、定义重载及区块生命周期事件合并入脏队列，低频有游标审计补漏。索引、队列和缓存随卸载／停服清理，不为每块普通外壳创建活动对象，不每 tick 扫描世界找控制器。未来接入现有全服预算时增加结构服务的公平份额，不另起一套可叠加的无限循环。
...
| M03b 真实形成入口 | M03a；控制器／角色方块、世界状态映射、正式预算队列及形成入口 | 专服按真实方块四向形成、错误角色／空腔、拆除重建、邻接／重叠；不以直接调用纯目录替代正式入口 |
| M03c 生命周期联合门 | M03b；事件失效、区块与正常重启、所有权恢复边界 | 内部区块卸载、两轮重建、控制器重放、正常重启与旧部件引用失效；无重复 ticker；M03 父门须联合完成 |
...
当前已完成 D16c2c 操作界面以及 M02a／b 一体机模板与预算扫描，见 10.45–10.47。网络主线下一步为 c3 真实玩家闭环，先明确最小授权事务，再验收实际登录、双玩家竞争和正常玩家文件重启。结构主线下一步为 M03，使用第 3.3 节的三尺寸模板接入真实方块、机器目录、排他占位和事件失效；仍不启用 M04 的生产与插件。主机核心材质与动画共用 M01 锚点，实际视觉状态同步及客户端验收见 M06。

用户已选定首台为蜂箱与离心一体机，并要求后续安装自动离心升级以直接产出离心结果。当前按 2.5 的插件首发方案推进，保留结构升级方块输入同一能力模型的扩展；机器尺寸和角色已在 M02 明确，单元容量与直出计费仍在各自进入门校准。尚未注册升级物品、形成一体机或启用直出生产。
