...
独立矩形多方块的 M01、M02、M03 已完成固定布局结构范围的联合验收，M03d 补齐稳定只读审计；M06a 静态状态、M06b1 展示帧与 M06b2 内部静态场景已通过真实客户端与专服检查，证据见 10.41、10.46–10.54。三尺寸一体机可形成并展示结构状态，生产／插件仍待 M04 的 D16c、D17 前置。下一步推进 M06b3 纯时间轴与活动生命周期，复杂生产演出按 9.2 分段实施；M 系列状态独立于 P3 验收，见 11.12。
...
**M02 首台模板合同（2026-09-24）：** `combined_apiary` 布局版本 1，局部宽×高×深为 `7×5×5`、`7×5×7`、`7×5×9`，仅三个候选，共用一个朝北锚点；改变深度不增加工作能力。控制器 `(3,1,0)`、管理／插件界面 `(3,2,0)` 朝前；内部主核 `(3,2,深/2)`，蜂箱单元 `(2,1,深/2)` 和离心单元 `(4,1,深/2)` 朝前，各唯一。能源口 `(0,1,2)` 朝左，输入／输出口 `(2,1,深-1)`／`(4,1,深-1)` 朝后。角和棱必须为框架，其余面为外壳或玻璃，其余内部格必须为空气。主核上方空腔留给未来结构升级布局；本版声明 `UPGRADE/NETWORK_PORT` 角色但不允许其替换空气或外壳，启用须升级布局版本并做相应交接验收。主核使用单格中心作为视觉锚点；M01 的一般几何模型仍可表达非单格核心。
...
### 9.2 一体机内部生产演出（2026-09-25，按 M06b／c 分步实施）

目标是一个可在观察窗内观看的连续场景：**蜜蜂飞入蜂箱 → 蜂箱放大并淡出 → 蜜脾块显现并压扁出液 → 资源迸发 → 短暂悬浮 → 飞入箱子**。这里“透明度升高”对应不透明度 alpha 从 1 降至 0。这是对已经提交的工作结果的视觉表达，时长不代表真实生产耗时，形成结构本身不能启动生产演出。

**编排与空间。** 使用 M01 的局部连续坐标、四向变换和有限视觉盒。蜂箱、压榨点、资源散布范围与收纳箱是同一内部场景的锚点，整体缩放到各尺寸可用空腔；最大蜂箱尺寸、液流和最远资源轨迹都必须包含在盒内。M06a 实拍显示正面中央管理接口遮挡部分空腔，M06b 先在四向及斜侧视角验证锚点可见性，可调整场景偏移或后续接口模型，不能靠穿墙绘制消除遮挡，也不能悄悄改变布局 1 的结构规则。

| 阶段 | 首轮调参时间（共 160 游戏 tick，约 8 秒） | 客户端表现 |
| --- | --- | --- |
| 飞入 | 0–30 | 烘焙蜜蜂模型，沿有切线朝向的曲线飞向蜂箱入口；翅膀摆动与少量上下起伏独立插值 |
| 放大、淡出 | 30–55 | 蜂箱缓动放大，蜂影在入口消隐；蜂箱 alpha 降低，蜜脾块交叉显现 |
| 压榨 | 55–85 | 蜜脾沿竖直轴缩小，横向适度展开，缩放始终大于零；有限液流网格／液滴从压榨位置流出 |
| 资源迸发 | 85–95 | 短促光环和有界粒子产生爆发感，资源按确定性种子向外散开 |
| 悬浮 | 95–125 | 资源缓慢转动与小幅浮动，数个图标代表一次合并展示，不按实际产量生成图标 |
| 收纳 | 125–160 | 资源依次沿曲线进入场景内箱子并淡出，箱盖可开合，结束后回到静态状态 |

表中时间是首轮美术原型参数，需真客户端调参，不是已经验收的效果。完整演出需要真实蜂箱与离心／直出链的已提交完成事件；仅蜂箱或普通离心活动选择对应片段，不能暗示未执行的工序。箱子是视觉收纳符号，不读取附近箱子、不承诺发生世界物流运输；真实资产已经在服务端结算完成。

**渲染与职责。** 蜜蜂外观使用客户端烘焙的原版模型／可替换模型和纹理，显式控制姿态；不创建或 tick 蜜蜂实体。蜂箱、蜜脾、箱子和资源由共享模型／网格与 PoseStack 变换绘制；不创建临时世界方块、ItemEntity、流体方块或调用世界爆炸。液滴、冲击环和资源也是有界场景元素，不把每个装饰物注册成长期世界实体。重用 Minecraft 可访问的资源引用，不复制第三方模组的美术或引入 AE2、LDLib、Veil 作为独立机的必需依赖。

M06b 将接收／生命周期、纯时间轴采样和 BER 绘制分开：时间轴输入为经过校验的展示描述、客户端游戏时间与 partial tick，输出阶段、姿态、缩放和 alpha；采样不读服务器对象、库存、世界邻居或写业务状态。资源模型缓存按资源重载代际失效，渲染器可共享烘焙模型但不能让两台机器共享可变姿态。透明蜂箱与蜜脾需要能够传递逐顶点 alpha 的专用绘制路径，不能假定普通不透明方块模型会接受整体淡出。液流／发光层保留深度测试，分别验证透明排序、深度写入与玻璃观察窗的组合；禁用深度测试、无限剔除盒或每机强制 flush 都不是通用修复。

**事件与生命周期。** 服务端只从已提交工作派生有界展示描述：机器身份／代际、结构与视觉版本、布局／活动样式、单调事件序号、时间锚与确定性种子，以及限量的物品／流体外观标识。编码上限在 M06b 首次开放包时落实；不发送完整库存、任意组件 NBT 或超长资源列表。不能安全简化的组件模型／第三方自定义物品渲染使用通用图标后备。首次跟踪把当前事件序号设为已知基线，不重演历史产量；旧代际、旧版本和重复序号不重放，迟到事件超过演出窗口直接丢弃。只有正在跟踪且在视觉距离内的玩家接收展示数据。

单机仅保留当前演出与一个可合并的后继摘要；256 倍等高加速合并完成事件，不能排队补播全部生产或把 8 秒动画加速成闪烁。b3b 合并规则为最新合法描述替换后继，不拼接不同活动、不累计资源或数量；当前结束后先按后继原时间锚检查有效期，未过期才从本地零相位播放，否则丢弃。首帧显式设置序号基线；未取得基线时首事件只建立基线，下一条新事件才可演出。离开视距可跳过演出，资源重载、世界切换、机器拆除／失效和区块卸载清除瞬态状态；单机真正暂停时随游戏时间冻结，相位不能改用墙钟继续推进；多人打开菜单不等于暂停世界。活动期间游戏时间回退或相邻推进超过 20 tick 时取消当前和后继，保留已消费序号；纯空闲期间没有逐机 ticker，不能把长时间空闲后的新事件误判为时间跳变。演出中断不退款、不补产，任何收纳回调都不能触发结算。

**预算与后续门。** 首轮上限目标：每场景最多 8 种展示资源、24 个资源图标与 48 个短寿命粒子；近景同时活动场景最多 16 个，其余用静态状态或较低细节。距离／视锥剔除先于姿态和网格构造，按材质批量提交；网络另有每机合并间隔与每查看者总字节／频率预算，不能让每机各领一份总预算。上述是待测起始上限，M06c 以 1／16／64 台及高加速同场景的帧时间、分配量、draw call、网络量和缓存清理确定最终值，不能据此宣称性能已达标。

M06b 先完成有版本快照、内部静态场景和可独立验证的时间轴／生命周期；真实工作触发要等 M04。M06c 再分蜜蜂与蜂箱、蜜脾压榨、资源迸发收纳三段实现并联合调试，随后覆盖关闭／降低动画、资源包重载、暂停／时间跳变、晚加入／卸载重进、遮挡剔除、Fast／Fancy／Fabulous 与 Iris 光影回退。每段保留 M06a 的状态标志，回退后仍能分辨结构状态；测试演出仅留开发探针，不能在正式版伪造生产。新增客户端设置必须同步当前配置界面和中英文文本。

**M06b 进一步拆分：** b1 先交付只含固定布局索引、方向、结构状态和身份／版本的原生 BE 展示帧，独立于权威 NBT；本次服务器会话共享单调发布号，不保存到世界，客户端按连接内版本拒绝旧帧并在区块卸载清理。结构帧仅在变化时借原版区块追踪更新，同位置同 tick 由原版合并，初始区块数据提供完整帧；编码不超过 256 字节，不含工作事件或资源列表，不增加独立发包 ticker。b2 再接内部静态场景、同源锚点和 BER；b3a 的有界描述及纯时间轴（10.55）和 b3b 的接收／生命周期（10.56）已分步交付；生产包和动画绘制仍走下列独立门。9.2 的生产事件、按查看者汇总的活动流限频／字节预算和 M04 触发在首次开放工作演出前另行验收，不能将静态结构帧的原版合并当作活动流总预算。

**静态场景到动画的空间门。** b2 的蜂箱／蜜脾／箱子均放在现有空气格，亮度取模型所在格的光照缓存；静态道具不表示已有产物或工作正在进行。第 2／3 层观察面可以按原模板选玻璃，不透明外壳继续遮挡。b3／M06c 引入蜂箱放大和液流／资源轨迹前，须重新验算最大运动包络、实体主核／单元的避让和有限视觉盒，不能因静态模型入盒就推断整段动画也安全。原版不透明模型只承担本步静态后备，后续 alpha 绘制仍按上文单独实现与验收。

**参考边界。** 本轮确认独立 AE2LT 参考仓库干净且 fast-forward 后仍为 `1d4589b6`；读取 `TianshuCoreEffectRenderer`、`CoreEffectAnimationState`、`CoreEffectRenderTypes`。采用游戏时间插值、共享局部锚点、有限剔除范围及着色器失败后备的原则；其循环核心与弱引用状态不能直接承担本项目的多阶段事件、显式生命周期和预算。其透明／加法层深度写入策略也须在“玻璃＋渐隐方块＋资源图标”场景重测，不能把已有核心特效当作本演出已兼容的证据。
...
**剩余门：** 当前 BER 仍绘制静态场景，描述注入只存在开发源集；M06c 才把采样姿态绘制成可见动画，继续按实际顶点验算包络及透明深度排序。16 台是有界入口上限，不是已完成 1／16／64 台帧耗或公平性验收；粒子、网络共享字节／频率预算、多人菜单、跨维度往返、Fast／Fabulous／Iris 和关闭／降低效果仍按联合门补验。M04 真实已提交工作尚未开放，M06／P3 父门未关闭。
...
| M06c 分段演出与联合门 | M06b；9.2 蜜蜂入箱、蜂箱淡出、蜜脾压榨、资源迸发悬浮收纳，再做连接材质和可选光影 | 单机暂停／多人菜单、资源重载、时间跳变、关闭／降低效果、图形模式、Iris 后备及 1／16／64 台与高加速预算实测 |
...
| AE2LT／`1d4589b6`，13.2 主路径，`TianshuMultiblockScanner/TianshuCoreEffectRenderer` | 同一局部核心位置驱动 BER 与有限剔除盒，视觉状态与业务分离 | 任意尺寸逐个覆盖区块；不依赖固定 7³ 的四角假设，不引 Veil；M01／M02／M06 |
...
当前已完成 D16c2c 操作界面、M01–M03 固定布局一体机结构、M03d 稳定只读审计、M06a 静态状态、M06b1 展示帧与 M06b2 内部静态场景，见 10.45–10.54。网络主线下一步仍为 D16c3 真实玩家闭环，先明确最小授权事务，再验收实际登录、双玩家竞争和正常玩家文件重启。多方块 M06b3a／b 的有界描述、纯时间轴与活动生命周期已完成（10.55／10.56）；下一步按 M06c 先绘制蜜蜂飞入与蜂箱渐隐，再推进压榨和资源收纳；9.2 已明确蜜蜂入箱到资源收纳的分镜、透明绘制、事件合并和客户端预算。M04 生产／插件继续等待 D16c、D17，真实生产演出不能由结构形成冒充；完整 M06、动画兼容与帧耗仍待后续联合验收。
