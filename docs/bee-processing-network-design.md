...
独立矩形多方块已完成 M01–M03 固定布局结构、M03d 稳定审计及 M06a／b 的前期展示验证。布局 2 与大型几何核心已分步验收（10.58／10.59）：六种尺寸兼容旧三种模板，环境动画不代表生产，具体合同和分步见 9.2。生产／插件仍等待 M04 的 D16c、D17 前置；M 系列状态独立于 P3 验收，见 11.12。
...
### 10.59 M06c2a 抽象几何核心（2026-09-26，本步已验收）

**合同与接入：** 接续工作区中的分层核心／双轨道原型，正式 BER 只读有效结构帧和客户端时间，不消费六阶段工作事件、不伪造生产。26 个带面板子块、两条 64 段环和六节点共 1,904 顶点；每帧共享 16 台详细额度，溢出为 48 顶点静态核心，同机重复绘制不重扣。空间按六模板预计算，共享三角函数表，无世界／邻居扫描、每机 tick 或累计姿态缓存；卸载／重载清理帧引用。成型单元保留低矮底座，原空核心模型改为四角静态标记，方块碰撞／资产职责保持不变。

**参考及差异：** ae2lt 2.1.0-beta.4 TianshuCoreEffectRenderer／CoreEffectAnimationState 与 §4.3.2 → 核心锚点、层转、局部游戏时间和实体／加法层分离 → 本项目独立顶点色网格、全客户端帧预算、显式清理，不添加 AE2／Veil 依赖，不复制第三方资源或墙钟相位。纯时间采样先对 long 取模再加小数，长期世界仍有 partial tick 精度；环境动效暂不需要低通工作态，留 M04 接入真实工作后验收。

**验证：** 新增 5 项纯测试覆盖六布局空间、长期时间／倒退／非有限输入、共享预算／重复领取／清理；全量 974 项中 972 通过、2 项既有跳过，build／verifyReleaseArtifact／开发探针编译通过。真实客户端 `build/network-probe-m06-core[-ae2]-20260926/results/machine-client.json` 两种依赖均通过：24 台六布局四向、每 20 tick 捕获完整 18 次层转的实际顶点，全部位于归一化单位球和有限盒；姿态平衡、16 台共享配额及溢出、真实暂停／恢复、故障／旧帧、卸载重进、重载和退出清理通过。每组保存 24 张正面及 4 张斜视截图；本轮人工查看高版正面、低版斜视，确认深度遮挡仍有效。专服 `m06-core-server[-ae2]-20260926` 恢复／正常退出通过，无客户端类加载失败。

**剩余门：** 顶点预算与实际截图不是 FPS、draw call 或 MSPT 结论；设置开关在 c2b，Fast／Fabulous／Iris、跨维度／多人、任意资源包、连接材质及 1／16／64 台性能在 c2c。M04 仍未开放，旧活动纯模型保留但未连接正式渲染；本步没有发布工作事件包。
...
| M06c2a 几何核心（已验收，10.59） | M02c、M06b1；分层核心／轨道／公转节点、静态标记后备、客户端每帧共享预算 | 六布局四向实际顶点、截图、剔除／暂停／卸载／重载与专服类加载；不产生生产事件 |
...
当前主线下一步仍为 D16c3：先交付显式可撤销的最小授权事务，再验收真实登录、双玩家竞争及正常玩家文件重启。多方块当前按 9.2 转向抽象几何核心，M02c 布局 2 与 M06c2a 核心渲染已分别验收，下一步完成 M06c2b 效果设置；六阶段具象原型仅保留历史证据。M04 生产／插件继续等待 D16c、D17；环境旋转不表示生产，完整 M06、兼容与性能联合门仍未关闭。
