...
单机仅保留当前演出与一个可合并的后继摘要；256 倍等高加速合并完成事件，不能排队补播全部生产或把 8 秒动画加速成闪烁。离开视距可跳过演出，资源重载、世界切换、机器拆除／失效和区块卸载清除瞬态状态；单机真正暂停时随游戏时间冻结，相位不能改用墙钟继续推进；多人打开菜单不等于暂停世界。时间回退或大跳变重置演出。演出中断不退款、不补产，任何收纳回调都不能触发结算。
...
**M06b 进一步拆分：** b1 先交付只含固定布局索引、方向、结构状态和身份／版本的原生 BE 展示帧，独立于权威 NBT；本次服务器会话共享单调发布号，不保存到世界，客户端按连接内版本拒绝旧帧并在区块卸载清理。结构帧仅在变化时借原版区块追踪更新，同位置同 tick 由原版合并，初始区块数据提供完整帧；编码不超过 256 字节，不含工作事件或资源列表，不增加独立发包 ticker。b2 再接内部静态场景、同源锚点和 BER；b3a 先交付有界描述及纯时间轴（10.55），b3b 再接事件接收与客户端生命周期。9.2 的生产事件、按查看者汇总的活动流限频／字节预算和 M04 触发在首次开放工作演出前另行验收，不能将静态结构帧的原版合并当作活动流总预算。
...
| M06b3b 活动接收与生命周期 | b3a；首次基线、旧代际／重复／迟到拒绝、单当前与单后继合并、暂停／跳时与清理 | 客户端卸载／重载／失效／世界切换探针；开放活动包前仍需共享网络预算与 M04 完成事件 |
...
| OmniSequence／`333b78f`，`.tmp_omnisequence_src`，`SequenceCrownAnimation` | working／completion 触发客户端瞬态插值，无库存／工作存档 | 增加代际与事件序号、资源重载清理、时间跳变测试，不能只靠弱引用宣称不存在泄漏；M06 |
...
当前已完成 D16c2c 操作界面、M01–M03 固定布局一体机结构、M03d 稳定只读审计、M06a 静态状态、M06b1 展示帧与 M06b2 内部静态场景，见 10.45–10.54。网络主线下一步仍为 D16c3 真实玩家闭环，先明确最小授权事务，再验收实际登录、双玩家竞争和正常玩家文件重启。多方块 M06b3a 的有界事件描述与纯时间轴已完成（10.55）；下一步为 M06b3b 接收、合并、暂停／时间跳变和取消清理，随后按 M06c 分段绘制；9.2 已明确蜜蜂入箱到资源收纳的分镜、透明绘制、事件合并和客户端预算。M04 生产／插件继续等待 D16c、D17，真实生产演出不能由结构形成冒充；完整 M06、动画兼容与帧耗仍待后续联合验收。
