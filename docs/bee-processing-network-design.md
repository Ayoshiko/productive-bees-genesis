...
状态：P2（D09–D12）和 P3／D13–D15 已验收，详见 10.21–10.29；D16a 共享 FE 已以 `6b56ecd` 推送，D16b1 自动蜂箱已以 `d861ff2` 推送。D16b2a 增量库存和保留凭据已以 `ce05eec` 推送，见 10.32；D16b2b 基础自动离心、输入规则及蜂箱串联见 10.33。D16b2c1 总预算与 c2 维护费机制已验收（10.34–10.35）；c3 跨 JVM 恢复／冷初始化及 D16c 最小玩家闭环尚未完成，P3 未退出。日期：2026-09-21。D01 独立机测量基线：`f2ae0b8ba586d1a9f8ee6d512d959b4bc2fb1de6`，模组版本 `1.0.8`；开发分支为 `bees-processing-network/1.21.1`，维护分支为 `main-neo/1.21.1`。
...
D01 已增加独立专服现状基线，采样证据见第 10.4 节。基础网络已实现到 D16b2c2，下述优化假设与公开测试门槛仍待同场景对照验证；当前独立机测量不能证明新网络收益。性能分析使用 SparkMCP 读取原始 profile，保留其完整 tick 指标与夹具事件计时的区别。
...
静态产品策略按服务器、配方代际和策略版本缓存，停服清理，最多缓存八个策略版本；各网络持有自己的准入注册表，不共享动态发现。已有 `PbProductPolicyCompiler` 冷启动／重载编译仍是不可抢占的全配方编译步骤，当前软时间预算不能保证其耗时上限；D16b2c／D30 须单独测量并决定是否进一步拆分。库存与规则扫描已分步，不能据此宣称所有初始化成本已预算化。暂停、切换候选及完成作业会释放旧库存视图，普通推进、占料和结算沿用世界自动保存，不逐 tick 请求整域写盘。
...
D16b2 进一步按依赖拆分：**b2a** 增量可用量索引和保留占料凭据（10.32，已推送 `ce05eec`）；**b2b** 真实自动离心、输入规则／库存候选和蜂箱串联（10.33）；**b2c** 再拆为 c1 跨子系统总预算（10.34）、c2 核心维护费机制（10.35）、c3a 冷初始化测量及预算边界、c3b 跨 JVM 自动恢复；每项先单独验收，再核对 D16b 的联合矩阵。b2b 对目标库存规则明确拒绝自动接单，完整目标维护依赖 D22 的在制统计；不修改或删除 D08 领域水位算法。各子步检查后独立提交。
...
P1、P2、D13–D15、D16a／b1／b2a／b2b 已完成各自范围；D16b2c1／c2 已完成各自范围，c3 及 D16c 仍待验收。子步完成不会自动解除父步骤或阶段的门槛。
...
P2、D13–D15、D16a／b1 已验收推送；D16b2a 增量库存已推送，D16b2b 自动离心与输入保留规则串联见 10.33。下一步 D16b2c 补跨 JVM 自动恢复矩阵、核心维护费、跨子系统总预算并测量冷初始化，再由 D16c 补正式玩家供给、有限取出与装笼事务。目标库存规则须在 D22 接完整在制／在途量，本阶段不绕过其限制。继续复用固定计算内核，不使用已清空升级的代理机器能力，也不执行隐藏物理 ticker。实施前更新所用参考仓库，重点复核 ECO 输入代际、DataEnergistics 精确余量交付及本地 Mekanism 工作条件。新装入蜜蜂须分配新 beeId，不能复用已迁走蜜蜂的旧槽身份。P3 验收前保持默认关闭，性能收益仍须同场景 Spark／MSPT 实测。
