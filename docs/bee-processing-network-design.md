...
状态：P2（D09–D12）已完成，见 10.21；P3／D13 已以 `583ecee` 提交推送，见 10.22–10.23；D14 基础蜂箱逐位喂食、共享组与迁移恢复已实现，见 10.24。D15a 共享内核已提交 `2495301`，D15b1 作业事务与严格恢复见 10.26；D15b2a／b2b 见 10.27–10.28，D15b2c 已提交 `0195452`，见 10.29。D16a 统一 FE 已以 `6b56ecd` 提交推送，见 10.30；D16b1 核心开始／暂停、全服生产预算与基础蜂箱自动调度见 10.31。D16b2 自动离心串联及 D16c 最小玩家闭环尚未完成，P3 未退出。日期：2026-09-21。D01 独立机测量基线：`f2ae0b8ba586d1a9f8ee6d512d959b4bc2fb1de6`，模组版本 `1.0.8`；开发分支为 `bees-processing-network/1.21.1`，维护分支为 `main-neo/1.21.1`。
...
D16b2 进一步按依赖拆分：**b2a** 增量可用量索引和保留占料凭据（10.32）；**b2b** 真实自动离心、规则／库存候选和蜂箱串联；**b2c** 跨 JVM 自动恢复矩阵、核心维护费及跨子系统总预算。b2a 的领域验收不替代 b2b 的真实生产验收，各子步检查后独立提交。
...
P1／D08、P2／D09–D12 和 P3／D13–D15 已完成各自验收并推送；D15b2 真实接入及跨 JVM 恢复见 10.27–10.29。D16a 已以 `6b56ecd` 推送，D16b1 见 10.31，D16b2／c 尚待完成。D03–D08 各自经过聚焦测试、自审和全量回归再单独提交，当时未启用机器接管。P1 退出检查确认领域算法和真实 API 能组合；D09 的权威 checkpoint 须包含交易明细、动态发现、暂存及规则状态，不能直接把 D06 的余额查询 Snapshot 当完整存档。任何阶段只完成接口而未通过行为验收时均保持未完成状态。
...
P2、D13–D15 已验收推送；D16a 共享能源已推送，D16b1 自动基础蜂箱与公平预算见 10.31。下一步 D16b2 接自动离心和保留规则，补跨 JVM 自动恢复矩阵、维护费和总预算，再由 D16c 补核心供给、有限取出与装笼事务。继续复用固定计算内核，不使用已清空升级的代理机器能力，也不执行隐藏物理 ticker。实施前更新所用参考仓库，重点复核 ECO 输入代际、DataEnergistics 精确余量交付及本地 Mekanism 工作条件。新装入蜜蜂须分配新 beeId，不能复用已迁走蜜蜂的旧槽身份。P3 验收前保持默认关闭，P5 再形成完整终端和 AE2 闭环；性能收益仍须同场景 Spark／MSPT 实测。
